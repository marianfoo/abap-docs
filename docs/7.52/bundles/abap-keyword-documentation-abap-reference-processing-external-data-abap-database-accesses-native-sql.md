...
-   A name granted using AS can also be used in Open SQL after the addition CONNECTION and in the statements [COMMIT CONNECTION](javascript:call_link\('abapcommit_rollback_connection.htm'\)) and [ROLLBACK CONNECTION](javascript:call_link\('abapcommit_rollback_connection.htm'\)), as long as it is in uppercase letters.
