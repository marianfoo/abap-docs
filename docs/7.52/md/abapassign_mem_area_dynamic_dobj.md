...
Dynamic output of the content of any system field. The validity of the input is checked before it is dynamically assigned with (name) to field symbol syfield via an application of classes of [RTTI](javascript:call_link\('abenrtti_glosry.htm'\) "Glossary Entry") on structure SYST
...
-   Usually addition [RANGE](javascript:call_link\('abapassign_range.htm'\)) must be used to defined the area, within which it is possible to work with INCREMENT.
...
After the ASSIGN statement, the field symbol points to the fourth component col4. See the example for addition [RANGE](javascript:call_link\('abapassign_range.htm'\)).
