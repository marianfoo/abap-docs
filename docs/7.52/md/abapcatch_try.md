...
Introduction of a CATCH block of a [TRY](javascript:call_link\('abaptry.htm'\)) control structure in which exceptions can be handled.
...
-   Of a [RESUME](javascript:call_link\('abapresume.htm'\)) statement is executed in the CATCH block, processing resumes after the statement that raised the exception.
