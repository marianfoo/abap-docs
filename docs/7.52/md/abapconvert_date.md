...
-   These forms of the CONVERT statement are not allowed in classes. They were previously used for sorting by date fields in internal tables and [extracts](javascript:call_link\('abenextract_glosry.htm'\) "Glossary Entry") for changing the sort direction. These have now been replaced by the ASCENDING and DESCENDING additions of the [SORT](javascript:call_link\('abapsort_itab.htm'\)) statement. If necessary, the complement on nine can be formed using the following [TRANSLATE](javascript:call_link\('abaptranslate.htm'\)) statement:
    

dat2 = dat1.
TRANSLATE dat2 USING '09182736455463728190'.

-   Do not mistake the variant of the statement CONVERT DATE shown here with the statement [CONVERT DATE ... INTO TIME STAMP](javascript:call_link\('abapconvert_date_time-stamp.htm'\)) used for creating time stamps.
