...
The CONVERT TEXT statement is designed to fill an index column (in internal tables) by which the rows of the table can be sorted by locale. Since internal tables can also be sorted by locale directly by using the AS TEXT addition of the [SORT](javascript:call_link\('abapsort_itab.htm'\)) statement, CONVERT TEXT is especially useful in the following cases:
