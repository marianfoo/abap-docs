...
[Open SQL](javascript:call_link\('abenopen_sql_glosry.htm'\) "Glossary Entry") statement. Deletes rows from a database table or a classic view.

Additions

-   [FROM dbtab*|*view*|*(target\_syntax)](javascript:call_link\('abapdelete_target.htm'\))
    Specifies the database table and/or view statically or dynamically.
    
-   [USING CLIENT](javascript:call_link\('abapdelete_target.htm'\))
...
-   [CLIENT SPECIFIED](javascript:call_link\('abapdelete_target.htm'\))
