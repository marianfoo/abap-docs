...
The [Open SQL](javascript:call_link\('abenopen_sql_glosry.htm'\) "Glossary Entry") statement FETCH extracts the requested rows (using the addition INTO or APPENDING) from the results set of the [database cursor](javascript:call_link\('abendatabase_cursor_glosry.htm'\) "Glossary Entry") (associated with the cursor variable dbcur) from the current cursor position and assigns these rows to the data objects specified in the results set.
