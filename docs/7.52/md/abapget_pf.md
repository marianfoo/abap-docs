...
Use the addition EXCLUDING to insert the function codes (which are inactive in the current GUI status) row by row into the internal table fcode. For fcode, an internal table with a [flat](javascript:call_link\('abenflat_glosry.htm'\) "Glossary Entry") character-like row type can be specified. Function codes are only defined if they have been deactivated with the relevant addition of the statement SET PF-STATUS. Statically deactivated function codes in the GUI status are not defined.
