...
-   Alongside the reference operator [REF](javascript:call_link\('abenconstructor_expression_ref.htm'\)) and the addition [REFERENCE INTO](javascript:call_link\('abapread_table_outdesc.htm'\)), the statement GET REFERENCE is the only option available to statements for internal tables to create [stack references](javascript:call_link\('abenstack_reference_glosry.htm'\) "Glossary Entry"). Stack referencescan become invalid if the referenced data object is deleted.
