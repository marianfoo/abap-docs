...
-   The statement LOOP AT GROUP is matched by the expression [FOR ... IN GROUP](javascript:call_link\('abenfor_in_group.htm'\)), which means that its function can often be expressed more elegantly by [table comprehensions](javascript:call_link\('abentable_comprehension_glosry.htm'\) "Glossary Entry") or [table reductions](javascript:call_link\('abentable_reduction_glosry.htm'\) "Glossary Entry").
