...
This variant of the statement MODIFY works essentially like the [short form](javascript:call_link\('abapmodify_obsolete.htm'\)) of the Open SQL statement, but belongs, along with the addition VERSION, to the number of [obsolete access statements](javascript:call_link\('abendb_access_obsolete.htm'\)), for which for dbtab, the name of a database table or a [classic view](javascript:call_link\('abenclassical_view_glosry.htm'\) "Glossary Entry") must be specified, beginning with "T" and comprising no more than five characters.
