...
-   In assignments between reference variables, the [assignment operator \=](javascript:call_link\('abenequals_operator.htm'\)) can only be used for [up cast](javascript:call_link\('abenup_cast_glosry.htm'\) "Glossary Entry")s in which the static type of source\_ref is more specific than or the same as the static type of destination\_ref.
