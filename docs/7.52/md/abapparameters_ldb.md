...
With this addition, the parameter has to be assigned to a node of the structure of the logical database. If a node belongs to type T, the addition TABLE can be used instead of NODE. A description of the node types can be found under the statement [NODES](javascript:call_link\('abapnodes.htm'\)).
