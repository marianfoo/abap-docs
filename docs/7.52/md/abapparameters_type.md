...
If the predefined ABAP type p is specified, DECIMALS can be used to define the number of [digits after the decimal point](javascript:call_link\('abenfractional_portion_glosry.htm'\) "Glossary Entry") dec. If DECIMALS is specified, the number of digits after the decimal point is 0. If specified, the same applies to DECIMALS as in the statement [TYPES](javascript:call_link\('abaptypes_simple.htm'\)).
...
[Enumeration types](javascript:call_link\('abenenumerated_type_glosry.htm'\) "Glossary Entry") cannot be specified, regardless of the [basic type](javascript:call_link\('abenbase_type_glosry.htm'\) "Glossary Entry").
