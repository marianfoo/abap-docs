...
Determines the average value of the contents of column col in the results set or in the current group. The data type of the column must be numeric. The data types DF16\_RAW and DF34\_RAW and the obsolete types DF16\_SCL and DF34\_SCL are not allowed. Data type INT8 is only permitted in conjunction with addition [AS dtype](javascript:call_link\('abapselect_avg_as.htm'\)). The data type of the result for [decimal floating point numbers](javascript:call_link\('abendecfloat_glosry.htm'\) "Glossary Entry") is the corresponding data type (DF16\_DEC or DF34\_DEC) - otherwise the type is either FLTP, a platform-dependent subtotal or it is determined by addition [AS dtype](javascript:call_link\('abapselect_avg_as.htm'\)). SQL expressions cannot be specified as arguments.
