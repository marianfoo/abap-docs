...
[ABAP - Keyword Documentation](javascript:call_link\('abenabap.htm'\)) →  [ABAP - Reference](javascript:call_link\('abenabap_reference.htm'\)) →  [Processing External Data](javascript:call_link\('abenabap_language_external_data.htm'\)) →  [ABAP Database Accesses](javascript:call_link\('abenabap_sql.htm'\)) →  [Open SQL](javascript:call_link\('abenopensql.htm'\)) →  [Open SQL - Reads](javascript:call_link\('abenopen_sql_reading.htm'\)) →  [SELECT clauses](javascript:call_link\('abenselect_clauses.htm'\)) →  [SELECT - FROM](javascript:call_link\('abapfrom_clause.htm'\)) →  [SELECT - data\_source](javascript:call_link\('abapselect_data_source.htm'\)) → 
...
In this case, the data of the internal table is accessed on the application server and the table is handled like a table in the [table buffer](javascript:call_link\('abentable_buffer_glosry.htm'\) "Glossary Entry"). This is possible for all database platforms.
...
-   No deep row types are allowed. An elementary row type cannot be a string or a reference type and a structured row type cannot contain any strings, reference types, or internal tables as components, with the following exception: An elementary row type or a component with the type [string](javascript:call_link\('abenbuiltin_types_character.htm'\)) is allowed if this is declared using a reference to the built-in dictionary type [SSTRING](javascript:call_link\('abenddic_builtin_types.htm'\)).
...
The data in the internal table is handled like data on the database even if it is not transported to the database. In particular, data with the type [string](javascript:call_link\('abenbuiltin_types_character.htm'\)) declared using a reference to the built-in dictionary type [SSTRING](javascript:call_link\('abenddic_builtin_types.htm'\)) is handled like text fields with fixed lengths in which trailing blanks are ignored.
...
-   An internal table as a data source of SELECT is also, however, a potential alternative to the statements [READ TABLE](javascript:call_link\('abapread_table.htm'\)) and [LOOP AT](javascript:call_link\('abaploop_at_itab.htm'\)) and hence makes Open SQL syntax possible that cannot otherwise be used. If this solution is used, it should be ensured that the data is evaluated on the application server and that no data is transported to the database system to be evaluated.
...
-   Access to an internal table as a data source of SELECT should not be seen as an alternative to the statements [READ TABLE](javascript:call_link\('abapread_table.htm'\)) and [LOOP AT](javascript:call_link\('abaploop_at_itab.htm'\)). More specifically, data should only be transported to the database when it is actually needed there.
