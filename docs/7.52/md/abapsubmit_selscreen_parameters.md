...
You can create and manage selection screen variants for every program in which selection screens are defined, either in the [ABAP Workbench](javascript:call_link\('abenabap_workbench_glosry.htm'\) "Glossary Entry") or during execution of the program by choosing Goto - Variants on a selection screen.
...
-   SIGN, OPTION, LOW, and HIGH must contain the values specified for the [selection table](javascript:call_link\('abenselection_table_glosry.htm'\) "Glossary Entry") columns that have the same names as the selection criteria. For parameters, the value must be specified in LOW and all other components are ignored.
