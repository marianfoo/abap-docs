...
The logical expression log\_exp is false. Also, no receiver is registered for APC messages in the current program and no registrations for AMC messages or [asynchronous function calls](https://help.sap.com/doc/abapdocu_752_index_htm/7.52/en-US/abenasynchronous_rfc_glosry.htm "Glossary Entry") exist when the additions MESSAGING CHANNELS or ASYNCHRONOUS TASKS are used.
...
If these additions are specified, this variant of the statement WAIT is combined with the variants [WAIT FOR MESSAGING CHANNELS](https://help.sap.com/doc/abapdocu_752_index_htm/7.52/en-US/abapwait_amc.htm) or [WAIT FOR ASYNCHRONOUS TASKS](https://help.sap.com/doc/abapdocu_752_index_htm/7.52/en-US/abapwait_until.htm). The program flow described in the variants is added to the program flow described above. The WAIT statement then also waits for AMC messages or for asynchronously called functions to be called and does not just wait for APC messages.
