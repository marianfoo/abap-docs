...
If these additions are specified, this variant of the statement WAIT is combined with the variants [WAIT FOR MESSAGING CHANNELS](javascript:call_link\('abapwait_amc.htm'\)) or [WAIT FOR ASYNCHRONOUS TASKS](javascript:call_link\('abapwait_until.htm'\)). The program flow described in the variants is added to the program flow described above. The WAIT statement then also waits for AMC messages or for asynchronously called functions to be called and does not just wait for APC messages.
