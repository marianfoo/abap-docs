...
If the parsing is successful, the XML document [document](javascript:call_link\('abenabap_ixml_lib_input_output.htm'\)) associated with the parser can be used to [access](javascript:call_link\('abenabap_ixml_lib_dom_access.htm'\)) the DOM saved in the memory.
