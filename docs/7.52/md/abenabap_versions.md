...
This version of ABAP is [obsolete](javascript:call_link\('abennon_unicode_obsolete.htm'\)) and is no longer supported in the current release. The version ID of a program of this type is initial. No [Unicode checks](javascript:call_link\('abenunicode_check_glosry.htm'\) "Glossary Entry") are performed in programs of this type.
