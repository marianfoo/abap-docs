...
-   When XML data is saved in strings or internal tables, it is best to use byte strings or byte-like row types and the UTF-8 code page for the representation. [sXML Library](javascript:call_link\('abenabap_sxml_lib.htm'\)) supports byte-like data only. [iXML Library](javascript:call_link\('abenabap_ixml_lib.htm'\)) and the statement [CALL TRANSFORMATION](javascript:call_link\('abapcall_transformation.htm'\)) support text strings and internal tables with character-like data types too.
