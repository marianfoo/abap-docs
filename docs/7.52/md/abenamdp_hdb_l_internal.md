...
The programming language L is only [documented](javascript:call_link\('abenllang_internal.htm'\)) internally at SAP. The following list contains special properties and additional restrictions for [AMDP methods](javascript:call_link\('abenamdp.htm'\)) implemented in L:
