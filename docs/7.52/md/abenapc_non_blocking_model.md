...
To stop [APC sessions](javascript:call_link\('abenapc_session_glosry.htm'\) "Glossary Entry") of this type from being blocked for inbound messages, the programming must run in [non-blocking mode](javascript:call_link\('abennon_blocking_model_glosry.htm'\) "Glossary Entry"). The following blocking statements are not allowed in non-blocking mode for APC:
