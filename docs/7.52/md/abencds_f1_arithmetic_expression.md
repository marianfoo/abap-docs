...
A \- in front of an operand multiplies it by -1. The data type of the operands must be numeric and be based on one of the [predefined data types](javascript:call_link\('abenddic_builtin_types.htm'\)) INT1, INT2, INT4, INT8, DEC, CURR, QUAN, or FLTP. The following can be specified:
...
-   [Predefined functions](javascript:call_link\('abencds_f1_builtin_functions.htm'\)) that return a numeric type
