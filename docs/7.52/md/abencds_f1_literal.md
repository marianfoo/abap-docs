...
-   A numeric literal numeric\_literal is a series of digits between 0 and 9. It can be directly prefixed by a + or \- sign and can contain a decimal point .) after the first digit. A numeric literal without a decimal point is handled as a field of the type [INT1](javascript:call_link\('abenddic_builtin_types.htm'\)), [INT2](javascript:call_link\('abenddic_builtin_types.htm'\)), or [INT4](javascript:call_link\('abenddic_builtin_types.htm'\)), depending on the value. There is no implicit handling as a field of type [INT8](javascript:call_link\('abenddic_builtin_types.htm'\)). A numeric literal with a decimal point is floating point number of type [FLTP](javascript:call_link\('abenddic_builtin_types.htm'\)). The value of a numeric literal cannot be greater than the value range of the valid types.
...
-   The specification of a literal prefixed with a sign is interpreted as an [arithmetic expression](javascript:call_link\('abencds_f1_arithmetic_expression.htm'\)) and can be specified wherever it is permitted.
