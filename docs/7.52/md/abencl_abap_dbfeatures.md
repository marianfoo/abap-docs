...
Access to [CDS table functions](javascript:call_link\('abencds_table_function_glosry.htm'\) "Glossary Entry") in Open SQL.

EXTERNAL\_VIEWS

[SAP HANA views](javascript:call_link\('abenhana_view_glosry.htm'\) "Glossary Entry")

Access to [external views](javascript:call_link\('abenexternal_view_glosry.htm'\) "Glossary Entry") in Open SQL.
...
Use of [internal tables](javascript:call_link\('abapselect_itab.htm'\)) as a [data source of queries](javascript:call_link\('abapselect_data_source.htm'\)) in Open SQL, if these are evaluated by the database.
