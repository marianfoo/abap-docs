...
-   IS\_RESUMABLE of type ABAP\_BOOL, which is used in a [CATCH](javascript:call_link\('abapcatch_try.htm'\)) block or [CLEANUP](javascript:call_link\('abapcleanup.htm'\)) block to show whether the exception is [resumable](javascript:call_link\('abenresumable_exception_glosry.htm'\) "Glossary Entry"), that is whether a CATCH block can be exited using [RESUME](javascript:call_link\('abapresume.htm'\)).
