...
An iterator is created for all nodes of a parsed XML and used to iterate. To identify nodes that are not elements, down casts are performed from IF\_IXML\_NODE to IF\_IXML\_ELEMENT. Both checks on regular down casts using the predicate expression [IS INSTANCE OF](javascript:call_link\('abenlogexp_instance_of.htm'\)) and down casts using the method QUERY\_INTERFACE of the interface IF\_IXML\_UNKNOWN are demonstrated.