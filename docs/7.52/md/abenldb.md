...
-   Logical databases should no longer be used as they are based on cross-program usage of global data, implicit subroutine calls and [reporting event control](javascript:call_link\('abenabap_processing_blocks.htm'\)), which means that they do not comply with modern concepts. No new logical databases should be created. Instead a relevant service should be made available using a global class.
