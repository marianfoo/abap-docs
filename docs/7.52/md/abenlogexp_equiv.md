...
-   The operator EQUIV implements an equivalence join. Negating a join like this using [NOT](javascript:call_link\('abenlogexp_not.htm'\)) corresponds to an XOR join (eXclusive OR). It is true if one of the expressions is true and the other one is false.
