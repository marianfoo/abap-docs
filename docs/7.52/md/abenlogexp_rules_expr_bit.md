...
-   [Relational operators for all data types](javascript:call_link\('abenlogexp_compare_all.htm'\))

-   [Relational operators for byte-like data types](javascript:call_link\('abenlogexp_bytes.htm'\))

-   [Relational operators for bit patterns](javascript:call_link\('abenlogexp_bitmasks.htm'\))
...
Each bit expression of a comparison expression is calculated based on the length of its longest operand, with shorter operands padded on the right with hexadecimal 0. All operands of the entire comparison expression are respected. The result of the bit expressions involved is contained within this length and, if necessary, a single operand is expanded to this length before the comparison, again by padding with hexadecimal 0 on the right. The comparison is then performed using the [comparison rule](javascript:call_link\('abencomparison_type.htm'\)) for a byte-like [comparison type](javascript:call_link\('abencomparison_type_glosry.htm'\) "Glossary Entry").
