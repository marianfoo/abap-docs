...
description: |
  In Native SQL, data can be transferred from the database to ABAP data objects in reads, and the other way round in the case of writes. -   In ADBC(https://help.sap.com/doc/abapdocu_752_index_htm/7.52/en-US/abenadbc.htm), ABAP data objects are bound using data references. -   ABAP data objects are
version: "7.52"
category: "database"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_752_index_htm/7.52/en-US/abennative_sql_type_mapping.htm"
...
-   Cut off values for character-like and byte-like types

-   Conversion rules different to those in ABAP

-   SQL errors leading to exceptions
...
cl\_demo\_output=>write( |Open SQL:   { host }| ).
EXEC SQL.
...
cl\_demo\_output=>write( |Open SQL: \\n{ result }| ).
TRY.
    EXEC SQL.
      UPDATE demo\_expressions SET numc1 = :host WHERE id = 'X'
    ENDEXEC.
