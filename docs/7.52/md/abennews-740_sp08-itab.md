...
A new variant [LOOP AT itab ... GROUP BY](javascript:call_link\('abaploop_at_itab_group_by.htm'\)) of the statement [LOOP AT itab](javascript:call_link\('abaploop_at_itab_variants.htm'\)) enables the rows in an internal table to be grouped and then the groups evaluated in a [group loop](javascript:call_link\('abengroup_loop_glosry.htm'\) "Glossary Entry"). The new statement [LOOP AT GROUP](javascript:call_link\('abaploop_at_group.htm'\)) for [member groups](javascript:call_link\('abenmember_loop_glosry.htm'\) "Glossary Entry") is used to access the rows in a group.
...
The new options for grouping internal tables exist alongside the existing [control level processing](javascript:call_link\('abencontrol_level_processin_glosry.htm'\) "Glossary Entry") functions and can replace them in many cases.
