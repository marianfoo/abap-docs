...
If one the new features listed above is used in an Open SQL statement, the syntax check is performed in a [strict mode](javascript:call_link\('abenopensql_strict_mode_751.htm'\)), which handles the statement more strictly than the regular syntax check.
...
A new [ABAP-specific session variable](javascript:call_link\('abenhana_session_variables.htm'\)) CDS\_CLIENT for the client ID is modified in Open SQL reads by addition [USING CLIENT](javascript:call_link\('abapselect_client.htm'\)). The [CDS session variable](javascript:call_link\('abencds_f1_session_variable.htm'\)) client is now linked to this HANA session variable.
...
-   When accessing [CDS views](javascript:call_link\('abencds_view_glosry.htm'\) "Glossary Entry") that use [session variables](javascript:call_link\('abencds_f1_session_variable.htm'\)), addition [USING CLIENT](javascript:call_link\('abapselect_client.htm'\)) can now be used. When accessing [CDS views](javascript:call_link\('abencds_view_glosry.htm'\) "Glossary Entry") that do not use the [session variables](javascript:call_link\('abencds_f1_session_variable.htm'\)) client, addition [CLIENT SPECIFIED](javascript:call_link\('abapselect_client.htm'\)) can now be used.