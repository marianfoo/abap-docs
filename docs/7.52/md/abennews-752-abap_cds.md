...
In a [LEFT OUTER JOIN](javascript:call_link\('abencds_f1_joined_data_source.htm'\)), an addition TO ONE or TO MANY can be specified for the cardinality. This addition is evaluated by an [SAP HANA database](javascript:call_link\('abenhana_database_glosry.htm'\) "Glossary Entry") as a note for optimization.

This option exists in earlier releases in ABAP CDS, but has been documented only from Release 7.52. In [Open SQL](javascript:call_link\('abennews-752-open_sql.htm'\)), the corresponding syntax was introduced in Release 7.52.
...
-   [CDS roles](javascript:call_link\('abencds_role_glosry.htm'\) "Glossary Entry") can now also be defined for [CDS table functions](javascript:call_link\('abencds_table_function_glosry.htm'\) "Glossary Entry"). When a CDS table function is accessed in Open SQL. The access conditions are evaluated by default.
