...
For cross-transaction objects, [shared objects](javascript:call_link\('abenshared_objects_glosry.htm'\) "Glossary Entry") exist that can be accessed by all programs of an application server at the same time.
...
As soon as an object has no more references and none of its methods is registered as an event handler, it can be deleted by [Garbage Collector](javascript:call_link\('abengarbage_collector_glosry.htm'\) "Glossary Entry"). This releases the identity of the object so that it can be taken by a new object.
