...
[Host expressions](javascript:call_link\('abenhost_expression_glosry.htm'\) "Glossary Entry") are ABAP expressions expr that can be specified in operand positions in Open SQL. expr is a [general expression position](javascript:call_link\('abengeneral_expr_position_glosry.htm'\) "Glossary Entry"). Any expressions or calls possible here, plus individual data objects, can be specified.
...
The host expressions of an Open SQL statement are evaluated from left to right and the results are passed to the database, like the content of [host variables](javascript:call_link\('abenopen_sql_host_variables.htm'\)). The following restrictions apply:
