...
The cardinality of the current association \_assoc is declared by specifying a number n or the character \* is specified in parentheses ( ). The numbers 1 and 2 can be specified for n. If specified, the cardinality overwrites the definition of the cardinality [\[min..max\]](javascript:call_link\('abencds_f1_association.htm'\)) of the current association with "to 1", "to 2", or "to \*".
...
Columns specified in the filter condition sql\_cond always refer to the target [target](javascript:call_link\('abencds_f1_association.htm'\)) of the association for which the condition is specified. No explicit name may be specified in front of a specified column using the [column selector](javascript:call_link\('abentable_comp_selector_glosry.htm'\) "Glossary Entry") ~.
