...
-   Fields of the types STRING and RAWSTRING ([LOBs](javascript:call_link\('abenlob_glosry.htm'\) "Glossary Entry")) from the results set can be assigned to reference variables for [LOB handles](javascript:call_link\('abenselect_into_lob_handles.htm'\)) as well as to strings. The static type of these reference variables must be one of the system classes or one of the system interfaces that support [streaming and locators](javascript:call_link\('abenstreams_locators.htm'\)) for Open SQL.
