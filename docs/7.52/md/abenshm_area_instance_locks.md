...
Once an exclusive lock has been set in an internal session, reads and changes on the area instance version and its objects are possible until the methods [DETACH\_COMMIT](javascript:call_link\('abenshm_cl_shm_area.htm'\)) or [DETACH\_ROLLBACK](javascript:call_link\('abenshm_cl_shm_area.htm'\)) are executed.
...
-   Closing of an internal session (except when using the statement LEAVE TRANSACTION (or function "/n")). The statement LEAVE TRANSACTION releases all exclusive locks using the method [DETACH\_ROLLBACK](javascript:call_link\('abenshm_cl_shm_area.htm'\)).
