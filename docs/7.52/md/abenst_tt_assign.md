...
An assignment between reference variables with tt:assign is possible only if the static type of the source variables is more special or the same as the static type of the target variables (up cast). In all other cases, the statement [tt:cast](javascript:call_link\('abenst_tt_cast.htm'\)) can be used to perform a down cast.
