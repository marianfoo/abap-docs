...
In the [static method](javascript:call_link\('abenst_tt_call-method_static.htm'\)) or [instance method](javascript:call_link\('abenst_tt_call-method_instance.htm'\)) called from an ST program, the attributes writer and reader can be used to access the XML stream where the serialization or deserialization is taking place.
