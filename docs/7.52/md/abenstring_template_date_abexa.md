...
The current date is output in all possible [country-specific date formats](javascript:call_link\('abencountry_formats.htm'\)) using parameter [COUNTRY](javascript:call_link\('abapcompute_string_format_options.htm'\)). For this purpose, in the setup and teardown help methods, temporary rows with the possible values of column DATFM are inserted into database table T005X and are deleted again once they have been used.