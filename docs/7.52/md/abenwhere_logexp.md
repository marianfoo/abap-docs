...
-   If [SQL expressions](javascript:call_link\('abensql_expression_glosry.htm'\) "Glossary Entry") other than column-only expressions are specified on the left side of a relational expression or the right side of a relational expression specifies items other than [host variables](javascript:call_link\('abenhost_variable_glosry.htm'\) "Glossary Entry") or [host expressions](javascript:call_link\('abenhost_expression_glosry.htm'\) "Glossary Entry"), the Open SQL statement bypasses [table buffering](javascript:call_link\('abensap_buffering_glosry.htm'\) "Glossary Entry").
