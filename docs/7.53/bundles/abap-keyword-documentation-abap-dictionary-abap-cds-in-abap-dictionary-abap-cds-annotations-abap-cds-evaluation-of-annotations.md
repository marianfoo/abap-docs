...
-   Inheritances apply especially to those [element annotations](javascript:call_link\('abencds_f1_element_annotations.htm'\)) of [associations](javascript:call_link\('abencds_association_glosry.htm'\) "Glossary Entry") published in the [SELECT list](javascript:call_link\('abencds_f1_select_list.htm'\)) using a path expression [path\_expr](javascript:call_link\('abencds_f1_path_expression.htm'\)). A [publication](javascript:call_link\('abencds_f1_select_list_association.htm'\)) of an association specified in a different CDS view inherits the element annotations of preceding publications.
