...
[Quick Reference](javascript:call_link\('abapclose_cursor_shortref.htm'\))
...
-   Initializing a cursor variable using the statement CLEAR is not enough to close a database cursor. If the value of the cursor variable in the program is known, the database cursor can be further processed using [FETCH](javascript:call_link\('abapfetch.htm'\)).
