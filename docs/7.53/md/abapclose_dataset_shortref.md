  

* * *

AS ABAP Release 753, ©Copyright 2019 SAP AG. All rights reserved.

[ABAP Keyword Documentation](javascript:call_link\('abenabap.htm'\)) →  [ABAP - Quick Reference](javascript:call_link\('abenabap_shortref.htm'\)) →  C

CLOSE DATASET - Quick reference

[Reference](javascript:call_link\('abapclose_dataset.htm'\))

Syntax

CLOSE DATASET dset.

Effect

Closes a file on the current [AS Instance](javascript:call_link\('abenapplication_server_glosry.htm'\) "Glossary Entry") [host computer](javascript:call_link\('abenhost_computer_glosry.htm'\) "Glossary Entry") that was specified in dset and opened previously using [OPEN DATASET](javascript:call_link\('abenapplication_server_glosry.htm'\) "Glossary Entry").