...
The formatting option DECIMALS does not override the [predefined format](javascript:call_link\('abenstring_templates_predef_format.htm'\)) specifying that thousands separators are not inserted.
...
As SPACE. However here the date format and time format are defined in accordance with according to the current [formatting setting](javascript:call_link\('abencountry.htm'\)) of the [language environment](javascript:call_link\('abenlanguage_environment_glosry.htm'\) "Glossary Entry") that can be set using [SET COUNTRY](javascript:call_link\('abapset_country.htm'\)).
...
-   In numbers, the formatting option COUNTRY overrides the [predefined setting](javascript:call_link\('abenstring_templates_predef_format.htm'\)) with respect to thousand separators.
