...
If the addition USING KEY is not used, the addition INDEX can only be used with [index tables](javascript:call_link\('abenindex_table_glosry.htm'\) "Glossary Entry") and determines the row to be deleted from the [primary table index](javascript:call_link\('abenprimary_table_index_glosry.htm'\) "Glossary Entry").
...
[Deleting Rows Using an Index](javascript:call_link\('abendelete_line_idx_abexa.htm'\))
...
The following loop deletes all rows in an internal table, since the short form of the statement [DELETE](javascript:call_link\('abapdelete_itab.htm'\)) always deletes the current first row.
...
This addition is required if the table key used by the LOOP is specified explicitly in the statement LOOP. It states explicitly that the current table row is deleted by the LOOP. No other key can be specified apart from the predefined name [loop\_key](javascript:call_link\('abaploop_at_itab_cond.htm'\)). If no explicit table key is specified for LOOP, the addition USING KEY loop\_key is optional.
...
Continue
![Example](exa.gif "Example") [Internal Tables, Deleting Rows Using the Index](javascript:call_link\('abendelete_line_idx_abexa.htm'\))