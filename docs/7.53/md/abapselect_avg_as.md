...
Length and decimal places must be specified with len and decimals. Literals or constant [host variables](javascript:call_link\('abenopen_sql_host_variables.htm'\)) of the [ABAP type](javascript:call_link\('abenbuilt_in_types_complete.htm'\)) b, s, i, or int8 can be specified for len and decimals. The value range of the data type dtype must cover the value range of the operand type:
...
-   For average values, the addition AS of AVG covers cases where aggregate expressions cannot currently be specified as operands of SQL expressions and the fact that FLTP cannot be cast to other numeric data types using [CAST](javascript:call_link\('abensql_cast.htm'\)).
