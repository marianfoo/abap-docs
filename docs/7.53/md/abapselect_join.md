...
-   Join expressions bypass [table buffering](javascript:call_link\('abensap_buffering_glosry.htm'\) "Glossary Entry"). Therefore they should not be applied to buffered tables. Instead it may be a good idea in these cases to use the addition [FOR ALL ENTRIES](javascript:call_link\('abenwhere_logexp_itab.htm'\)), which can access the [table buffer](javascript:call_link\('abentable_buffer_glosry.htm'\) "Glossary Entry").
