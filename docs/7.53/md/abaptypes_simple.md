...
If a [built-in data type](javascript:call_link\('abenpredefined_abap_type_glosry.htm'\) "Glossary Entry") abap\_type is specified, a non-generic elementary data type is defined. For abap\_type, all [built-in data types](javascript:call_link\('abenbuilt_in_types_complete.htm'\)) can be used except the internal types b and s.
