...
[Quick Reference](javascript:call_link\('abapunassign_shortref.htm'\))
...
-   The statement [CLEAR <fs>](javascript:call_link\('abapclear.htm'\)) does not initialize the field symbol and initializes the memory area that is assigned to the field symbol instead.
    
-   Unlike the initialization of reference variables, the statement UNASSIGN does not affect [Garbage Collector](javascript:call_link\('abengarbage_collector_glosry.htm'\) "Glossary Entry").
