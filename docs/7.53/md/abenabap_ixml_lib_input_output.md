...
description: |
  Input streams are used for the input of XML data into the parser(https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abenabap_ixml_lib_parse.htm) and output streams are used to pass XML data from the renderer(https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abenabap_ixml_lib_ren
version: "7.53"
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abenabap_ixml_lib_input_output.htm"
...
Input streams are used for the input of XML data into the [parser](https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abenabap_ixml_lib_parse.htm) and output streams are used to pass XML data from the [renderer](https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abenabap_ixml_lib_render.htm). A factory is needed to create a stream, which can be created using the iXML factory as follows:
...
-   iXML input streams can be specified as an XML source and iXML output streams can be used as an XML target for XSL transformations called using [CALL TRANSFORMATION](https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abapcall_transformation.htm).
...
The XML result of a transformation of an ABAP data object to the [asXML](https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abenabap_xslt_asxml.htm) format is used to create an iXML input stream. This stream is then transformed back again.
...
-   is used to address an XML document saved as [DOM](https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abenabap_ixml_lib_dom_access.htm),

-   can be bound to the [parser](https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abenabap_ixml_lib_parse.htm) to fill it,

-   can be used to construct new XML data or modify existing data,

-   can be passed to the [renderer](https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abenabap_ixml_lib_render.htm) to produce the output of the renderer.

Note

iXML documents can be defined as an XML target, iXML documents and their nodes can be specified as an XML source for XSL transformations called using [CALL TRANSFORMATION](https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abapcall_transformation.htm).
