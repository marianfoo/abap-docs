...
-   If this variant of an inheritance condition is the only access condition of an access rule and no CDS role is defined for the CDS entity cds\_entity, it is applied like a [full access rule](javascript:call_link\('abencds_dcl_role_grant_rule.htm'\)).
...
The optional addition REPLACING of INHERITING CONDITIONS can be used to replace the values assigned to the optional authorization fields in inherited [PFCG conditions](javascript:call_link\('abencds_f1_cond_pfcg.htm'\)):
