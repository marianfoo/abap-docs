...
-   Like in a CDS role that contains a [full access rule](javascript:call_link\('abencds_dcl_role_grant_rule.htm'\)), a CDS role that contains only literal conditions after any inheritance conditions are evaluated produces a syntax check warning. The access conditions of a CDS role should always be user-specific.
