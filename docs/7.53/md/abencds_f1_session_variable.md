...
-   In access to a CDS view using the ABAP SQL statement [SELECT](javascript:call_link\('abapselect.htm'\)) where the session variable client is used, the addition [CLIENT SPECIFIED](javascript:call_link\('abapselect_client.htm'\)) cannot be used. The addition [USING CLIENT](javascript:call_link\('abapselect_client.htm'\)) can be used and is respected in the session variable for the duration of the ABAP SQL statement.
