...
A different type of reference preserves objects until the available memory runs low. The class [CL\_ABAP\_SOFT\_REFERENCE](javascript:call_link\('abensoft_reference_glosry.htm'\) "Glossary Entry") is designed for these [soft references](javascript:call_link\('abensoft_reference_glosry.htm'\) "Glossary Entry"), but this class is currently still implemented in the same way as class CL\_ABAP\_WEAK\_REFERENCE.
