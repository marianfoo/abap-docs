...
The components of classes can be [accessed](javascript:call_link\('abenclass_components_addressing.htm'\)) internally and, depending on the visibility, also from outside the class. If accessed externally, [component selectors](javascript:call_link\('abencomponent_selector_glosry.htm'\) "Glossary Entry") must be used for addressing the components.
