...
To avoid this situation, use [SAP locks](javascript:call_link\('abensap_lock_glosry.htm'\) "Glossary Entry") or an unambiguous file name, such as [UUIDs](javascript:call_link\('abenuuid_glosry.htm'\) "Glossary Entry").

Note

When multiple AS Instancess in [AS ABAP](javascript:call_link\('abensap_nw_abap_glosry.htm'\) "Glossary Entry") try to access a file at the same time, conflicts can still occur despite SAP locks. This is the case, for example, when the operating system buffers data before writing it to a file.