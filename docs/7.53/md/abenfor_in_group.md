...
-   In the variant FOR ... IN GROUP without GROUPS OF, the expression evaluates the members of the current group in the same way as the variant [FOR ... IN itab](javascript:call_link\('abenfor_in_itab.htm'\)) evaluates the rows of the internal table itab. The result is the rows of the group.
...
The condition [WHERE ( log\_exp )](javascript:call_link\('abenfor_cond.htm'\)) can be used to restrict the evaluated rows of the group. Conditions other than this static WHERE condition cannot be specified.
...
Displays a list of the total occupied seats for each airline. A LOOP with the addition [GROUP BY](javascript:call_link\('abaploop_at_itab_group_by.htm'\)) evaluates its groups in a table reduction using FOR flight IN GROUP <carrier>. Here, the total for each group is calculated for the column seatsocc.
