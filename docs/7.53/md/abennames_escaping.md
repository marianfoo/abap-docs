...
The character ! can be placed directly before a name to distinguish it from an [ABAP word](https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abenabap_words.htm) with the same name in a statement. With the exception of the first word, each word of a statement that is preceded by the escape character is interpreted as an operand, and not as an ABAP word, when the program is generated. The escape character itself is not part of a name and is ignored when the statement is executed.

Notes

-   The escape character ! may be required on rare occasions in which the compiler cannot tell the difference between an operand and a [reserved word](https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abenabap_words.htm) of the same name. Otherwise, it can be used for the documentation of operands in the source code.

-   The escape character ! cannot be used in front of [operators](https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abenoperator_glosry.htm "Glossary Entry"). For example, != cannot be specified instead of \=.
