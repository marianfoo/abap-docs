...
The statement [SYNTAX-CHECK](https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abapsyntax-check_for_itab.htm) has been enhanced so that the syntax check also continues after error messages occur, as long as the parameter ID = ERR is set. Any error messages are written to an internal table, which can be specified using TABLE. The collected errors also provide the return code SY-SUBRC = 4.
...
The statement [SCAN ABAP-SOURCE](https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abapscan.htm) has been enhanced so that fields of type C or STRING are permitted in the scanned source code, instead of an internal table. This means that an internal table no longer needs to be edited.
...
In the statement [GENERATE SUBROUTINE POOL](https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abapgenerate_report.htm), source code can now also be passed in an internal table of row type string.