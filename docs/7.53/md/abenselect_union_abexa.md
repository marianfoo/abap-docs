...
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abenselect_union_abexa.htm"
...
Four columns of the same type from result sets from three database tables DEMO\_JOIN1, DEMO\_JOIN2 and DEMO\_JOIN3 are combined to make multiple SELECT statements using [UNION](https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abapunion.htm). The database tables are filled in the static constructor.

-   The first statement shows the default behavior with addition DISTINCT. No rows are inserted from database table DEMO\_JOIN2 and one row is not inserted from database table DEMO\_JOIN3 because these rows already exist.
