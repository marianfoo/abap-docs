...
Elementary expression in ABAP SQL. An elementary expression is either a column [col](javascript:call_link\('abenopen_sql_columns.htm'\)) of a [data source](javascript:call_link\('abapselect_data_source.htm'\)) or a literal literal, a [host variable](javascript:call_link\('abenopen_sql_host_variables.htm'\)) dobj, or a [host expression](javascript:call_link\('abenopen_sql_host_expressions.htm'\)) @( expr ).
