...
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abenst_tt_root.htm"
...
To be able to access ABAP data, an ST program must contain at least one declaration of a data root outside a template. The data roots are the interfaces of the ST program to the ABAP data objects, specified in the statement [CALL TRANSFORMATION](https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abapcall_transformation.htm) as source or target fields. Data roots are defined using the statement tt:root. A name must be assigned to the data root with the name attribute. It can be typed with the attribute *\[*line-*\]*type.

The data roots declared on the [tt:transform](https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abenst_tt_transform.htm) level form the context of the [main template](https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abenst_tt_template_main.htm) and can be addressed directly within it. The data roots are not recognized in [subtemplates](https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abenst_tt_template_sub.htm). However, they can be bound to the local data roots of subtemplates when these templates are [called](https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abenst_tt_apply.htm).

Note

An ST program without a data root describes a constant XML fragment that does not access ABAP data. In the statement CALL TRANSFORMATION, the syntax requires source fields or target fields to be specified. Data objects specified when ST programs are called without data roots are ignored in serializations and not changed in deserializations. The example for [literal texts](https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abenst_tt_text.htm) uses ST programs without data roots.
...
The symbolic name is not case-sensitive and must be unique. The namespace also includes the data roots declared using [tt:parameter](https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abenst_tt_parameter.htm) and the variables declared using [tt:variable](https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abenst_tt_variable.htm). No data roots other than the ones specified here can be used in the statement CALL TRANSFORMATION.
...
Elementary ABAP types, types from the repository, and types of the same ST program that are defined with [tt:type](https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abenst_tt_type.htm) can be specified as the value of *\[*line-*\]*type. Type specifications are subject to the same restrictions as the statement [tt:value](https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abenst_tt_type.htm).

Accessing a Data Root

Access to a data root is described under [Addressing the Data Roots](https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abenst_addressing_root_node.htm). [Serializations and deserializations](https://help.sap.com/doc/abapdocu_753_index_htm/7.53/en-US/abenst_serial_deserial.htm) are subject to the general restriction that the content of a data root cannot be modified by serialization and only writes can be performed on a data root in deserializations.
