...
The command tt:write writes the value of a [variable](javascript:call_link\('abenst_tt_variable.htm'\)) or [parameter](javascript:call_link\('abenst_tt_parameter.htm'\)) to the target XML data.
...
The value of the variable (the parameter) is inserted into the XML data at the position of the command (if applicable, after map). Depending on the type of the variable or parameter, a mapping takes place in accordance with the [asXML format](javascript:call_link\('abenabap_xslt_asxml.htm'\)).
