...
From a single column table array, a three-column table itab is created whose column content is calculated from the respective row content in array. The local helper fields in this example are the work area x of the [FOR expression](javascript:call_link\('abenfor_in_itab.htm'\)) and the helper field off defined in a [LET expression](javascript:call_link\('abaplet.htm'\)).