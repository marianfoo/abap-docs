...
ICF sessions are used for [ICF](javascript:call_link\('abenicf_processing_glosry.htm'\) "Glossary Entry") processing, namely the processing of HTTP/HTTPS queries or SMTP queries sent to AS ABAP in handler class.

-   [APC sessions](javascript:call_link\('abenapc_session_glosry.htm'\) "Glossary Entry")

APC sessions are used for [APC](javascript:call_link\('abenapc_processing_glosry.htm'\) "Glossary Entry") processing, namely the processing of WebSocket queries or TCP queries sent to AS ABAP in handler class.
