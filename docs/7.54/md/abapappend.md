...
title: "APPEND"
description: |
  Quick Reference(https://help.sap.com/doc/abapdocu_754_index_htm/7.54/en-US/abapappend_shortref.htm) Syntax APPEND line_spec(https://help.sap.com/doc/abapdocu_754_index_htm/7.54/en-US/abapappend_linespec.htm) TO itab SORTED BY comp  result(https://help.sap.com/doc/abapdocu_754_inde
version: "7.54"
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_754_index_htm/7.54/en-US/abapappend.htm"
...
APPEND [line\_spec](https://help.sap.com/doc/abapdocu_754_index_htm/7.54/en-US/abapappend_linespec.htm) TO itab *\[*SORTED BY comp*\]* *\[* [result](https://help.sap.com/doc/abapdocu_754_index_htm/7.54/en-US/abapappend_result.htm)*\]*.
...
This statement appends one or more rows [line\_spec](https://help.sap.com/doc/abapdocu_754_index_htm/7.54/en-US/abapappend_linespec.htm) to an internal index table itab. It is appended so that a new last row is created with regard to the primary table index.

If itab is a [standard table](https://help.sap.com/doc/abapdocu_754_index_htm/7.54/en-US/abenstandard_table_glosry.htm "Glossary Entry"), SORTED BY can be used to sort the table in a specified way. [result](https://help.sap.com/doc/abapdocu_754_index_htm/7.54/en-US/abapappend_result.htm) can be used when inserting a single row to set a reference to the inserted row in the form of a field symbol or a data reference.
...
-   Rows are appended to [sorted tables](https://help.sap.com/doc/abapdocu_754_index_htm/7.54/en-US/abensorted_table_glosry.htm "Glossary Entry") only if they match the sort order and do not create duplicate entries (if the primary table key is unique).
    
-   No rows can be appended to [hashed tables](https://help.sap.com/doc/abapdocu_754_index_htm/7.54/en-US/abenhashed_table_glosry.htm "Glossary Entry").
...
Used correctly, this addition can produce ranking lists in descending order. This only works if a value greater than 0 is specified in the declaration of the internal table in the addition [INITIAL SIZE](https://help.sap.com/doc/abapdocu_754_index_htm/7.54/en-US/abaptypes_itab.htm). If the value 0 is specified for INITIAL SIZE, the statement APPEND is ignored when used with the addition SORTED BY.

The addition SORTED BY can be used only when a work area wa is specified and for a standard table. Also, wa must be [compatible](https://help.sap.com/doc/abapdocu_754_index_htm/7.54/en-US/abencompatible_glosry.htm "Glossary Entry") with the row type of the table. The component comp can be specified as shown in the section [Specifying Components](https://help.sap.com/doc/abapdocu_754_index_htm/7.54/en-US/abenitab_components.htm), however only a single component can be addressed using the object component selector, and no attributes of classes.
...
[Create ranking lists with unsorted filling](https://help.sap.com/doc/abapdocu_754_index_htm/7.54/en-US/abensort_guidl.htm "Guideline")
...
Continue
[APPEND - line\_spec](https://help.sap.com/doc/abapdocu_754_index_htm/7.54/en-US/abapappend_linespec.htm)
[APPEND - result](https://help.sap.com/doc/abapdocu_754_index_htm/7.54/en-US/abapappend_result.htm)
![Example](exa.gif "Example") [Internal Table, Append Rows](https://help.sap.com/doc/abapdocu_754_index_htm/7.54/en-US/abenappend_lines_abexa.htm)