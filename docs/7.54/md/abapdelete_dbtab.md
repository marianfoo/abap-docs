...
DELETE *{* FROM target *\[*connection*\]* *\[* [WHERE sql\_cond](javascript:call_link\('abapdelete_where.htm'\))*\]* *\[*[db\_hints](javascript:call_link\('abenosql_db_hints.htm'\))*\]*
                                  [*\[*ORDER BY ... *\[*OFFSET o*\]**\]* *\[*UP TO n ROWS](javascript:call_link\('abapdelete_where.htm'\))*\]* *}*
     *|* *{* target *\[*connection*\]* FROM [source](javascript:call_link\('abapdelete_source.htm'\)) *}*.
...
The [ABAP SQL](javascript:call_link\('abenopen_sql_glosry.htm'\) "Glossary Entry") statement DELETE deletes one or more rows from the database table or classic view specified in target. The rows to delete can be specified as follows:
...
The addition connection can be used to specify a [secondary connection](javascript:call_link\('abensecondary_db_connection_glosry.htm'\) "Glossary Entry").
