...
This variant of the statement MODIFY works essentially like the [short form](javascript:call_link\('abapmodify_obsolete.htm'\)) of the ABAP SQL statement, but is one of a number of [obsolete access statements](javascript:call_link\('abendb_access_obsolete.htm'\)) (alongside the addition VERSION), where dbtab must be the name of a database table or a [classic view](javascript:call_link\('abenclassical_view_glosry.htm'\) "Glossary Entry"), starting with "T" and comprising no more than five characters.
