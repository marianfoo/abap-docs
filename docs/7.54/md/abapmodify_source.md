...
A non-table-like data object wa can be specified after FROM in the statement [](javascript:call_link\('abapmodify_dbtab.htm'\))[MODIFY](javascript:call_link\('abapinsert_from_select.htm'\)) and an internal table or a parenthesized [subquery](javascript:call_link\('abenopen_sql_host_variables.htm'\)) can be specified after TABLE. The work area and the internal table can be specified as a [host variable](javascript:call_link\('abenopen_sql_host_variables.htm'\)) or a [host expression](javascript:call_link\('abenopen_sql_host_expressions.htm'\)). On the one hand the content of the data objects or the results set of the subquery determines whether the row(s) are inserted or changed, and on the other hand, which values are inserted or used for changes.
