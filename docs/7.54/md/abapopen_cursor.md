...
The [ABAP SQL](javascript:call_link\('abenopen_sql_glosry.htm'\) "Glossary Entry") statement OPEN CURSOR opens a [database cursor](javascript:call_link\('abendatabase_cursor_glosry.htm'\) "Glossary Entry") for the result set of the [main query](javascript:call_link\('abenmainquery_glosry.htm'\) "Glossary Entry") defined after FOR and links a cursor variable dbcur with this database cursor. The results set of the main query can be read with the statement [FETCH](javascript:call_link\('abapfetch.htm'\)).
...
-   A [host variable](javascript:call_link\('abenopen_sql_host_variables.htm'\)) dbcurdeclared with the special predefined data type [cursor](javascript:call_link\('abenbuilt_in_types_complete.htm'\)). A database cursor dbcur that has already been opened cannot be opened again.
...
![Example](exa.gif "Example") [Reading Data Using Cursors](javascript:call_link\('abenopen_cursor_abexa.htm'\))