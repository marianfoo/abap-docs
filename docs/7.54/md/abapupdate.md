...
UPDATE target *\[*connection*\]* [source](javascript:call_link\('abapupdate_source.htm'\)).

Effect

The [ABAP SQL](javascript:call_link\('abenopen_sql_glosry.htm'\) "Glossary Entry") statement UPDATE changes the content of one or more rows of the database table specified in target. The entries in [source](javascript:call_link\('abapupdate_source.htm'\)) determine which columns of which rows are changed, and how they are changed.
