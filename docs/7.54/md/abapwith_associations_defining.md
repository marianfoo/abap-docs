...
If specified, JOIN initiates the definition and publication of a [CTE association](javascript:call_link\('abencte_association_glosry.htm'\) "Glossary Entry") with the name \_assoc in the addition [WITH ASSOCIATIONS](javascript:call_link\('abapwith_associations.htm'\)) when a common table expression is defined in a [WITH](javascript:call_link\('abapwith.htm'\)) statement.

A CTE association associates the current common table expression +cte as a source data source with the target data source target specified in the definition of the CTE association using an ON condition [sql\_cond](javascript:call_link\('abenosql_expr_logexp.htm'\)). Any [data sources](javascript:call_link\('abapselect_data_source.htm'\)) visible and usable in this position can be specified for target.
...
-   The definition of a CTE association requires [strict mode from Release 7.54](javascript:call_link\('abenopensql_strict_mode_754.htm'\)).
