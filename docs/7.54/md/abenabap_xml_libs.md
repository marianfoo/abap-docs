...
The "integrated" XML Library provides methods for parsing XML data in XML 1.0 format to [DOM](javascript:call_link\('abendom_glosry.htm'\) "Glossary Entry") representation, for editing a DOM representation, and for rendering a DOM representation in XML data. iXML Library supports DTDs (Document Type Definitions).
...
Objects in these libraries can be used as the source or target of the statement [CALL TRANSFORMATION](javascript:call_link\('abapcall_transformation.htm'\)).
