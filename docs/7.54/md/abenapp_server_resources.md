...
-   Number of available dialog work processes: 1
    This standard value is 1 and can be set using the [profile parameter](javascript:call_link\('abenprofile_parameter_glosry.htm'\) "Glossary Entry") rdisp/rfc\_min\_wait\_dia\_wp . By default, a certain number of dialog work processes are kept free as reserve instances for other purposes, such as system logon or administration programs.
