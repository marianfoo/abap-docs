...
The ABAP Core Data Services (ABAP CDS) are the platform-independent implementation of the general [CDS](javascript:call_link\('abencds_glosry.htm'\) "Glossary Entry") concept for AS ABAP. ABAP CDS makes it possible to define semantic data models on the [standard AS ABAP database](javascript:call_link\('abenstandard_db_glosry.htm'\) "Glossary Entry"). Unlike the SAP HANA-specific variant [HANA CDS](https://help.sap.com/viewer/09b6623836854766b682356393c6c416/2.0.03/en-US), ABAP CDS is independent of the database system. The entities of the models defined in ABAP CDS provide enhanced access functions compared with existing database tables and views defined in ABAP Dictionary, making it possible to optimize ABAP SQL-based applications. This is particularly clear when an AS ABAP uses a [SAP HANA database](javascript:call_link\('abenhana_database_glosry.htm'\) "Glossary Entry"), since its in-memory characteristics can be implemented in an optimum manner.
...
-   [CDS Views](javascript:call_link\('abenddic_cds_views.htm'\))
    [CDS entities](javascript:call_link\('abencds_entity_glosry.htm'\) "Glossary Entry") that are based (from a technical perspective) on [CDS database views](javascript:call_link\('abencds_database_view_glosry.htm'\) "Glossary Entry") in ABAP Dictionary.
...
-   [CDS Hierarchies](javascript:call_link\('abenddic_cds_hierarchies.htm'\))
    [CDS entities](javascript:call_link\('abencds_entity_glosry.htm'\) "Glossary Entry") that create a [hierarchy](javascript:call_link\('abenhierarchy_glosry.htm'\) "Glossary Entry") from a data source.

-   [Abstract CDS Entities](javascript:call_link\('abenddic_cds_abstract_entities.htm'\))
    [CDS entities](javascript:call_link\('abencds_entity_glosry.htm'\) "Glossary Entry") that describe only type attributes and for which no instances of database objects are created.

The [CDS DDL](javascript:call_link\('abencds_ddl_glosry.htm'\) "Glossary Entry") in ABAP CDS also contains language elements that enhance CDS entities.
...
-   As well as the database-independent ABAP CDS, [HANA CDS](https://help.sap.com/viewer/09b6623836854766b682356393c6c416/2.0.03/en-US) are also available as another variant of the [CDS](javascript:call_link\('abencds_glosry.htm'\) "Glossary Entry") concept. HANA CDS are only available in the [SAP HANA database](javascript:call_link\('abenhana_database_glosry.htm'\) "Glossary Entry"). Entities in HANA CDS are developed using HANA-specific tools such as [SAP Web IDE for SAP HANA](javascript:call_link\('abensap_web_ide_for_sap_han_glosry.htm'\) "Glossary Entry") and not with the [ABAP Development Tools (ADT)](javascript:call_link\('abenadt_glosry.htm'\) "Glossary Entry").

-   The fact that ABAP CDS is platform-independent from a functional perspective does not mean that the performance when accessing [CDS entities](javascript:call_link\('abencds_entity_glosry.htm'\) "Glossary Entry") is the same for all platforms. See the [Performance Note](javascript:call_link\('abenabap_cds_perfo.htm'\)).
