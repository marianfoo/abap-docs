...
-   The time stamp functions enable time stamps to be edited on the database that are saved [as a packed number](javascript:call_link\('abentime_stamps_packed.htm'\)) in the ABAP-specific format in database tables. As in ABAP, other types of access to these fields interpret these time stamps as regular numeric values.
