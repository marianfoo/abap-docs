...
Exponentiation
The [COMPUTE](https://help.sap.com/doc/abapdocu_754_index_htm/7.54/en-US/abapcompute.htm) statement now supports the operator "\*\*". Previously, the exponential operation "X \*\* Y" was awkward to express. You either had to use repeated multiplication (if Y was an integer) or the expression "EXP( Y \* LOG( X ) )" for any exponent Y.
