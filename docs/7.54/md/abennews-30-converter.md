...
If there are multiple [master programs](javascript:call_link\('abenframe_program_glosry.htm'\) "Glossary Entry") for an include, this is indicated at the end of the list. Conversion proposals are still made, but they refer only to the current master program. The names of the FORM routines defined in this include appear in the list of conversion proposals underlined in red.
