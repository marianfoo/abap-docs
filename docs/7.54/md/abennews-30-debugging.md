...
There are now special procedures for handling system programs and system screens in ABAP/4 debugging. (System programs are programs with the status 'S' in the program attributes, system screens are the [screens](https://help.sap.com/doc/abapdocu_754_index_htm/7.54/en-US/abendynpro_glosry.htm "Glossary Entry") of system programs or screens that contain only system modules (module name SYST\-...).)
...
It is now possible to track the generation of a list. As soon as a list is generated, you can branch to the display by pressing the Display list pushbutton. The display shows all existing lines with their formats. The only exception is the current line, which is not formatted until after [NEW-LINE](https://help.sap.com/doc/abapdocu_754_index_htm/7.54/en-US/abapnew-line.htm).
