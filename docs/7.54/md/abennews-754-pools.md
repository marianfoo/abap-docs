...
The method name in [handler classes](https://help.sap.com/doc/abapdocu_754_index_htm/7.54/en-US/abenrpm_handler_class.htm) is now arbitrary. This generalization makes it possible to include multiple methods in a handler class and, for example, to implement an action using a method. This enables the behavior implementation of a business object without introducing a corresponding number of handler classes.

Modification 2

No Exceptions

Triggering exceptions in [BEHAVIOR](https://help.sap.com/doc/abapdocu_754_index_htm/7.54/en-US/abenrpm_handler_class.htm) methods is prohibited. The attempt RAISE EXCEPTION will lead to a runtime error.