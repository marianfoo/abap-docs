...
Specification of a [host expression](javascript:call_link\('abenhost_expression_glosry.htm'\) "Glossary Entry") in a ABAP SQL statement. [Host expressions](javascript:call_link\('abenhost_expression_glosry.htm'\) "Glossary Entry") are ABAP expressions expr that can be specified in operand positions in ABAP SQL. expr is a [general expression position](javascript:call_link\('abengeneral_expr_position_glosry.htm'\) "Glossary Entry"). Any expressions or calls possible here, plus individual data objects, can be specified.
...
-   In some operand positions in which host expressions are forbidden, but [literals](javascript:call_link\('abenabap_sql_host_literals.htm'\)) and [host variables](javascript:call_link\('abenopen_sql_host_variables.htm'\)) are allowed, the syntax @( literal ) and @( dobj ) can be used. It is handled as a literal or host variable rather than as an expression.
