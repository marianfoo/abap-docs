...
-   Currently, a [path expression](javascript:call_link\('abenopen_sql_path.htm'\)) in a [specified column](javascript:call_link\('abenopen_sql_columns.htm'\)) in a [query](javascript:call_link\('abenquery_glosry.htm'\) "Glossary Entry") cannot contain any CDS associations for entities with input parameters.
