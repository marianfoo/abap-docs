...
The strict mode of the syntax check in Release 7.40, SP08 applies to all ABAP SQL statements that use one of the following attributes introduced in [Release 7.40 SP08](javascript:call_link\('abennews-740_sp08-open_sql.htm'\)):
...
-   If the content of a [literal](javascript:call_link\('abenabap_sql_host_literals.htm'\)), a [host variable](javascript:call_link\('abenopen_sql_host_variables.htm'\)), or a host [expression](javascript:call_link\('abenopen_sql_host_expressions.htm'\)) read in an operand position needs to be converted to the target type, this is done using the [rules for lossless assignments](javascript:call_link\('abapmove_exact.htm'\)). If the assignment cannot be lossless, an exception is raised.
