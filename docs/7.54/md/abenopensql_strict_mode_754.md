...
-   Use of the [new data types](javascript:call_link\('abennews-754-ddic.htm'\)) from ABAP Dictionary in a [CAST](javascript:call_link\('abensql_cast.htm'\)) expression or after the addition [AS](javascript:call_link\('abapselect_avg_as.htm'\)) of the aggregate function AVG.
...
-   Use of the hierarchy navigators [HIERARCHY\_DESCENDANTS\_AGGREGATE](javascript:call_link\('abenselect_hierarchy_desc_agg.htm'\)) and [HIERARCHY\_ANCESTORS\_AGGREGATE](javascript:call_link\('abenselect_hierarchy_ancs_agg.htm'\)).
...
The strict mode in Release 7.54 covers all rules of the [strict mode in Release 7.53](javascript:call_link\('abenopensql_strict_mode_753.htm'\)), plus the following rules:

-   An alias name defined using AS for columns of the results set in a [SELECT](javascript:call_link\('abapselect_list.htm'\)) list or for data sources in the [FROM](javascript:call_link\('abapfrom_clause.htm'\)) clause must comply with the [naming conventions](javascript:call_link\('abennaming_conventions.htm'\)) for internal program names and the name table\_line cannot be used. More specifically, an alias name cannot contain a minus character (\-).
