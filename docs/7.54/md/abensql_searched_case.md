...
If a complex CASE is used in a [condition](javascript:call_link\('abenasql_cond.htm'\)) of an ABAP SQL statement, the [client column](javascript:call_link\('abenclient_column_glosry.htm'\) "Glossary Entry") of a client-specific data source of a query or of the target of a write statement cannot be used as the operand of a WHEN condition due to [implicit client handling](javascript:call_link\('abenopen_sql_client_handling.htm'\)).
...
Continue
![Example](exa.gif "Example") [SQL Expressions, Complex CASE](javascript:call_link\('abensql_expr_searched_case_abexa.htm'\))