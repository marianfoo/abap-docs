...
A valid [time stamp](javascript:call_link\('abentime_stamp_glosry.htm'\) "Glossary Entry") in a [time stamp field](javascript:call_link\('abentimestamp_field_glosry.htm'\) "Glossary Entry") with the [time stamp type](javascript:call_link\('abentimestamp_type_glosry.htm'\) "Glossary Entry") utclong is formatted as "yyyy-mm-dd hh:mm:ss.fffffff" in accordance with SQL standard ISO 9075. A blank is placed between the date and time and the period (.) is used as the default decimal separator. An initial time stamp field is represented by a string of 27 blanks. A time stamp field whose value is greater than or equal to the last second or is between "9999-12-31 23:59:59.0000000" and "9999-12-31 23:59:59.9999999" is represented as "9999-12-31 23:59:59.9999999".

This setting can be overwritten using formatting options like [TIMESTAMP](javascript:call_link\('abapcompute_string_format_options.htm'\)), [COUNTRY](javascript:call_link\('abapcompute_string_format_options.htm'\)), or [DECIMALS](javascript:call_link\('abapcompute_string_format_options.htm'\)).
