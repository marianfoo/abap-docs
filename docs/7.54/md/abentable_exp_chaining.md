...
-   No table expressions can be specified in a writing position for a read-only internal table. This also applies when the object component selector is used to access another object. As before, [standalone method calls](javascript:call_link\('abapcall_method_static_short.htm'\)) a not possible using a read-only internal table.
