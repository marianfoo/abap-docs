...
This example demonstrates the addition REDIRECT of [WITH ASSOCIATIONS](javascript:call_link\('abapwith_associations.htm'\)) where the redirect is made to the current CTE.
...
As in the example [Publishing Association with a Redirect](javascript:call_link\('abenwith_assocs_redirect_abexa.htm'\)), a common table expression +cte accesses the CDS view DEMO\_CDS\_PUBLISH\_ASSOC\_1 and publishes its CDS association \_demo\_join2. Here, however, the redirect is made to the current common table expression and not to a preceding common table expression. This expression specifies the target data source demo\_join2 of the published CDS association twice as an additional data source under the names src1 and src2.
