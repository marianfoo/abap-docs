...
[WRITE TO, Cutoff Behavior](javascript:call_link\('abenwrite_cutoff_abexa.htm'\))
...
-   For [time stamps in packed numbers](javascript:call_link\('abentime_stamps_packed.htm'\)) whose output is produced using the addition [TIME ZONE](javascript:call_link\('abapwrite_to_options.htm'\)), elements are cut off the following order in the case of the statement [WRITE](javascript:call_link\('abapwrite-.htm'\)):
...
Continue
![Example](exa.gif "Example") [WRITE TO, Cutoff Behavior](javascript:call_link\('abenwrite_cutoff_abexa.htm'\))