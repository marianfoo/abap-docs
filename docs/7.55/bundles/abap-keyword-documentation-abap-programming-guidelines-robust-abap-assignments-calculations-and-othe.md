...
In a local context you can normally directly access the [data objects](javascript:call_link\('abendata_type_obj_guidl.htm'\) "Guideline") of superordinate more global contexts. For example, it is possible in a method to perform writes on the attributes of its class and on any global data of the current program.
