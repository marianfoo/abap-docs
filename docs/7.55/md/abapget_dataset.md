...
dset expects a character-like data object containing the [physical name](javascript:call_link\('abenphysical_filename_glosry.htm'\) "Glossary Entry") of the file. The file must already be open, otherwise a catchable exception of the class CX\_SY\_FILE\_OPEN\_MODE is raised.
...
Access mode. Possible values in attr are "I", "O", "A", and "U" for files that were opened for reading, writing, appending, and processing. The associated addition of the statement OPEN DATASET is [FOR access](javascript:call_link\('abapopen_dataset_access.htm'\)).
