...
If the built-in ABAP type p is specified, DECIMALS can be used to define the number of [digits after the decimal point](javascript:call_link\('abendecimal_place_glosry.htm'\) "Glossary Entry") dec. If DECIMALS is not specified, the number of decimal places is 0. If DECIMALS is specified, the same applies as in the statement [TYPES](javascript:call_link\('abaptypes_simple.htm'\)).
