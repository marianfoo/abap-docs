...
-   The key fields of a [secondary table key](javascript:call_link\('abensecondary_table_key_glosry.htm'\) "Glossary Entry"), however, are only read-only, while the secondary table is used. This is the case in [LOOP](javascript:call_link\('abaploop_at_itab.htm'\)) loops and when using the [MODIFY](javascript:call_link\('abapmodify_itab.htm'\)) statement where the secondary key is specified after USING KEY. Otherwise the key fields are not read-only.
