...
Length and decimal places must be specified with len and decimals. [Literals](javascript:call_link\('abenabap_sql_literals.htm'\)) or [host constants](javascript:call_link\('abenopen_sql_host_variables.htm'\)) of the [ABAP type](javascript:call_link\('abenbuilt_in_types_complete.htm'\)) b, s, i, or int8 can be specified for len and decimals. The value range of the data type dtype must cover the value range of the operand type:
