...
-   The comparison is based on the full row content, which means that the number of rows in the result set is determined by the [SELECT list](javascript:call_link\('abapselect_list.htm'\)). If, for example, all key fields of a DDIC database table are specified directly as Columns in a [SELECT](javascript:call_link\('abenopen_sql_columns.htm'\)) list, there can be no rows that occur more than once. The other extreme is the case where a SELECT list contains a single [host variable](javascript:call_link\('abenopen_sql_host_variables.htm'\)) or a single literal. Here, all rows are deleted except one.
