...
When the user chooses a pushbutton in the application toolbar, the runtime environment raises the event [AT SELECTION-SCREEN](javascript:call_link\('abapat_selection-screen.htm'\)) and the associated function code is passed to the component ucomm of the interface work area sscrfields.
...
-   To assign icons, a [tooltip](javascript:call_link\('abentool_tip_glosry.htm'\) "Glossary Entry"), and appropriate text to the pushbuttons, a data object can be defined of the structured type SMP\_DYNTXT in ABAP Dictionary. The ID of the icon, a tooltip, and the associated text must be assigned to the components of this data object. The content of the whole structure must then be assigned to the component functxt\_0n of the interface work area sscrfields.
