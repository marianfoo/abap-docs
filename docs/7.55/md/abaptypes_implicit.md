...
-   If len or dec is not specified for the ABAP types c, n, p, and x, the length is set to the [type-compliant standard length](javascript:call_link\('abenbuilt_in_types_complete.htm'\)). No decimal places are set for p. This is only allowed outside of classes or interfaces.
