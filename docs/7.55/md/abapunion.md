...
3.  For creating the result set of a parenthesized [subquery](javascript:call_link\('abensubquery_glosry.htm'\) "Glossary Entry") in a [relational expression](javascript:call_link\('abenosql_stmt_logexp.htm'\)), of the definition of a [common table expression](javascript:call_link\('abencommon_table_expression_glosry.htm'\) "Glossary Entry") after [WITH](javascript:call_link\('abapwith.htm'\)), or of an [INSERT](javascript:call_link\('abapinsert_dbtab.htm'\)) statement.
