...
[Implicit ABAP SQL client handling](javascript:call_link\('abenopen_sql_client_handling.htm'\)) applies to the WHERE clause. The [client column](javascript:call_link\('abenclient_column_glosry.htm'\) "Glossary Entry") of a client-dependent data source cannot be used as an operand in the WHERE condition.
...
-   If one of the columns involved in the logical expression [sql\_cond](javascript:call_link\('abenosql_stmt_logexp.htm'\)) contains the [null value](javascript:call_link\('abennull_value_glosry.htm'\) "Glossary Entry"), its result is unknown and no row is included in the result set.
