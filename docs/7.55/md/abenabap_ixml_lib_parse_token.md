...
The static type of the reference variable token\_parser is then the interface IF\_IXML\_TOKEN\_PARSER. A token parser requires the same input parameters as a [normal parser](javascript:call_link\('abenabap_ixml_lib_parse.htm'\)) and the [error analysis](javascript:call_link\('abenabap_ixml_lib_parse_error.htm'\)) is also the same.
