...
-   A JSON reader can, like every XML reader, be used as the source of the statement [CALL TRANSFORMATION](javascript:call_link\('abapcall_transformation.htm'\)), which allows direct serialization of JSON data to ABAP data objects.
...
-   A JSON writer can, like every XML writer, be used as the target of the statement [CALL TRANSFORMATION](javascript:call_link\('abapcall_transformation.htm'\)), which allows direct serialization of ABAP data objects to JSON data.
