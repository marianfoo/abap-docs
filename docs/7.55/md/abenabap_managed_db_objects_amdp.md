...
-   Instances of the [CDS-managed DDIC views](javascript:call_link\('abencds_mngdddic_view_glosry.htm'\) "Glossary Entry") of [CDS DDIC-based views](javascript:call_link\('abencds_v1_view_glosry.htm'\) "Glossary Entry") defined in ABAP CDS.
...
-   When a [CDS-managed DDIC view](javascript:call_link\('abencds_mngdddic_view_glosry.htm'\) "Glossary Entry") of a [CDS DDIC-based view](javascript:call_link\('abencds_v1_view_glosry.htm'\) "Glossary Entry") is accessed, its database-specific instance must be known. This applies in particular to CDS DDIC-based views with input parameters.
...
-   Care must be taken that all session variables used in a [CDS view entity](javascript:call_link\('abencds_session_variable_v2.htm'\)) or a [CDS DDIC-based view](javascript:call_link\('abencds_session_variable_v1.htm'\)) have the correct value. This applies in particular to the session variable client, to which the HANA session variable CDS\_CLIENT is assigned and not CLIENT.
