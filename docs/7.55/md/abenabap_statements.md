...
ABAP statements consist of the following [tokens](javascript:call_link\('abentoken_glosry.htm'\) "Glossary Entry") and always end with a period (.).
...
-   [Obsolete syntax forms](javascript:call_link\('abentokenization_obsolete.htm'\)) can still occur outside classes, where the separators between the tokens can be omitted.
