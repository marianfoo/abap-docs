...
-   Operands of to overloaded [general numeric functions](javascript:call_link\('abennumerical_functions.htm'\)), and of the overloaded power function [ipow](javascript:call_link\('abenpower_function.htm'\)), are used to determine the calculation type.
...
-   In the case of arithmetic expressions as operands of [relational expressions](javascript:call_link\('abenrelational_expression_glosry.htm'\) "Glossary Entry") in [logical expressions](javascript:call_link\('abenlogical_expression_glosry.htm'\) "Glossary Entry") joined using [Boolean operators](javascript:call_link\('abenboolean_operator_glosry.htm'\) "Glossary Entry"), the rule above only applies to the relational expression in question. The calculation type is not determined while skipping the Boolean operators AND, OR, or EQUIV.
