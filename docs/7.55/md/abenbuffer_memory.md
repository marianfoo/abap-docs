...
Table buffering takes place in the [shared memory](https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abenshared_memory_glosry.htm "Glossary Entry") of each AS instance. The [table buffer](https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abentable_buffer_glosry.htm "Glossary Entry") consists of a central management structure, an alphabetical directory of tables, and a data area. The individual areas of a [generically buffered](https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abenbuffer_generic_buffering.htm) table or DDIC database view are managed as individual [fully buffered](https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abenbuffer_complete_buffering.htm) tables. The buffered data is stored in the data area as [internal tables](https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abeninternal_table_glosry.htm "Glossary Entry"). The data area is managed using SAP Memory Management.
...
-   The management of single record buffering slightly less efficient as generic or full buffering. In single record buffering, the rows are loaded into the internal table in the data area gradually. In generic buffering and full buffering, all data of a table or view is loaded in a single step and sorted on the database.

-   When saved to memory, [null values](https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abennull_value_glosry.htm "Glossary Entry") are handled as such and are not transformed to type-dependent initial values.

Buffer Monitor

The buffer monitor is an SAP Memory Management tool (transaction ST02) used, among other things, to analyze the [table buffer](https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abentable_buffer_glosry.htm "Glossary Entry") of the current AS instance. This includes tasks such as:
...
-   Displaying detailed information about the [table buffer](https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abentable_buffer_glosry.htm "Glossary Entry").
...
For more information, see the Buffering documentation in SAP Help Portal.