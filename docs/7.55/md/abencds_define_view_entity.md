...
The addition [ROOT](javascript:call_link\('abencds_define_root_view_v2.htm'\)) is optional. It defines the CDS view entity as root entity of a [RAP business object](javascript:call_link\('abenrap_bo_glosry.htm'\) "Glossary Entry") in the [ABAP RESTful Application Programming Model](javascript:call_link\('abenarap_glosry.htm'\) "Glossary Entry").

Hints

-   [ABAP annotations](javascript:call_link\('abencds_view_entity_anno.htm'\)) can be used to assign further technical and semantic attributes to a view for evaluation by the ABAP runtime environment. [Framework-specific annotations](javascript:call_link\('abenfrmwrk_annotation_glosry.htm'\) "Glossary Entry") can be used to assign further semantic attributes to a view that are evaluated by other SAP frameworks.
