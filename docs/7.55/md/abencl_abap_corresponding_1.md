...
-   To achieve the same results for standalone in assignments between structures as in the operator [CORRESPONDING](javascript:call_link\('abencorresponding_constr_arg_type.htm'\)) without the addition BASE, an initial structure can be assigned to the parameter destination.

-   In reflexive assignments between components of the same object as in [MOVE-CORRESPONDING](javascript:call_link\('abapmove-corresponding.htm'\)), it should be noted that the processing order is not defined and that a call of the method EXECUTE cannot be used to swap the content of two components. See the executable example [Reflexive Component Assignments](javascript:call_link\('abenreflexive_corresponding_abexa.htm'\)).
