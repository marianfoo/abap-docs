...
-   The system class [CL\_ABAP\_CORRESPONDING](javascript:call_link\('abencl_abap_corresponding.htm'\)) is available for assignments between structures or internal tables with a dynamic mapping rule.
...
Specification of the [pseudo component](javascript:call_link\('abenpseudo_component_glosry.htm'\) "Glossary Entry") table\_line as the right side of a mapping rule.
