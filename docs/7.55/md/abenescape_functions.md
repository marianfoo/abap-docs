...
title: "Syntax"
description: |
  ... escape( val = text(https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abenstring_functions_val.htm) format = format ) ... Effect This function returns the content of the character string in text(https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abenstring_functions_val.htm),
version: "7.55"
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abenescape_functions.htm"
...
... escape( [val = text](https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abenstring_functions_val.htm) format = format ) ...

Effect

This function returns the content of the character string in [text](https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abenstring_functions_val.htm), and masks certain special characters with escape characters according to a rule specified in format.
...
[String Functions, escape for HTML](https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abenstring_function_escape_abexa.htm)
...
All characters with codes as of x80 are converted to their [UTF-8](https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abenutf8_glosry.htm "Glossary Entry") representation. Depending on the character, one to four bytes are represented in the form %hh, where hh is the hexadecimal value of a byte.
...
The program DEMO\_ESCAPE\_REGEX demonstrates the replacement rules of the format E\_REGEX for regular expressions. The [special characters of regular expressions](https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abenregex_posix_syntax_specials.htm) are prefixed by the associated escape character \\. Control characters with the codes x08, x09, x0A, x0B, x0C, and x0D are each replaced by \\b, \\t, \\n, \\v, \\f, and \\r.
...
-   URLs and URIs: Format E\_XSS\_URL. All characters except \[0-9\], \[a-z\], \[A-Z\], \*, \-, ., and \_ are transformed to %hh, where hh is the hexadecimal value of the code. All characters with codes as of x80 are converted to their [UTF-8](https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abenutf8_glosry.htm "Glossary Entry") representation. Depending on the character, one to four bytes are represented in the form %hh, where hh is the hexadecimal value of a byte.
...
-   escape used with rules for XSS is recommended as protection against cross site scripting, but might not be sufficient in some cases. For example, it may be best to use a whitelist to check an unsafe URL, so that phishing attacks can be detected in addition to XSS. To completely prevent code injections, JavaScript should never be created dynamically from unsafe sources.

Example

The result of the following masking is German#x3a;#x22;#xf6;#x22;.
...
[String Functions, escape for XSS](https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abenstring_function_esc_xss_abexa.htm)
...
-   Cause: A character cannot be converted in a conversion to UTF-8. This can only occur with characters from the [surrogate area](https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abensurrogate_area_glosry.htm "Glossary Entry"). The position and code of the character is specified in the exception object.
