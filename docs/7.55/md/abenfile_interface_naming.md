...
-   If an AS ABAP is distributed across multiple [AS instances](javascript:call_link\('abenapplication_server_glosry.htm'\) "Glossary Entry"), which themselves exist as instances on different host computers, the file interface generally accesses the individual file systems of these host computers. A shared file system can be configured for the host computers, to enable multiple AS instances to access the same files.

-   If different [AS instances](javascript:call_link\('abenapplication_server_glosry.htm'\) "Glossary Entry") of an AS ABAP exist as instances on a single host computer, the profile parameter DIR\_HOME has a different instance number for each AS instances by default. To enable shared access to files, the profile parameter must be modified or separate paths must be specified.
