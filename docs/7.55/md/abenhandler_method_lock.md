...
The LOCK implements the handler that locks entities. However, the [lock properties](javascript:call_link\('abenbdl_lock.htm'\)) lock master or lock dependent must be specified in the [behavior definition](javascript:call_link\('abencds_behavior_definition_glosry.htm'\) "Glossary Entry") of a [RAP business object](javascript:call_link\('abenrap_bo_glosry.htm'\) "Glossary Entry"). The method lock is called automatically before a change operation (for example, update).

The method name method\_name is arbitrary. Using this generalization, it is possible to accommodate multiple LOCK methods in a single [handler class](javascript:call_link\('abenabp_handler_class.htm'\)).
