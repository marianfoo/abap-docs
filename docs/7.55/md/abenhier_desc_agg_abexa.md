...
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abenhier_desc_agg_abexa.htm"
...
Calls the hierarchy navigator [HIERARCHY\_DESCENDANTS\_AGGREGATE](https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abenselect_hierarchy_agg_navis.htm) where the [CDS hierarchy](https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abencds_hierarchy_glosry.htm "Glossary Entry") DEMO\_CDS\_PARENT\_CHILD specified as a source is joined with a DDIC database table DEMO\_CHILD\_NUM and an internal table [@itab](https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abapselect_itab.htm) using the addition JOIN. The aggregate functions evaluate the column NUM of the joined data sources. The results of the join expressions used are also displayed.

-   In the first part, the table DEMO\_CHILD\_NUM is filled to produce a 1:1 relationship between the lines of the hierarchy and its own lines. The result of the hierarchy navigator is the same as in the executable example DEMO\_HIERARCHY\_AGGREGATE, but here the number values are taken from the joined table.

-   In the second part, the internal table @itab contains the lines of the DDIC database table DEMO\_CHILD\_NUM and further lines that also match the criteria for descendant nodes for nodes of the hierarchy. This example shows how these additional lines, produced in the result set of the join, are evaluated in the aggregate functions.

The statement [ASSERT](https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abapassert.htm) shows that the use of the [hierarchy generator](https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abenhierarchy_generator_glosry.htm "Glossary Entry") [HIERARCHY](https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abenselect_hierarchy_generator.htm) and a semantically identical [CDS hierarchy](https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abencds_hierarchy_glosry.htm "Glossary Entry") as the source of the hierarchy navigator produces the same results.