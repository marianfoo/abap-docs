...
This example demonstrates [host expressions](javascript:call_link\('abenopen_sql_host_expressions.htm'\)) on the right side of a [relational expression](javascript:call_link\('abenosql_stmt_logexp.htm'\)) of a [WHERE](javascript:call_link\('abapwhere.htm'\)) condition.
...
A table expression is evaluated as a host expression on the right side of an [SQL condition](javascript:call_link\('abenosql_stmt_logexp.htm'\)). The result is used for the comparison with the column specified on the left side.