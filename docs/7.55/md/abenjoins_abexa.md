...
description: |
  This example demonstrates inner and outer joins(https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abapselect_join.htm) as well as cross joins(https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abapselect_join.htm) in the SELECT statement. Source Code REPORT demo_joins NO STANDA
version: "7.55"
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abenjoins_abexa.htm"
...
This example demonstrates inner and outer [joins](https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abapselect_join.htm) as well as [cross joins](https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abapselect_join.htm) in the SELECT statement.
...
-   Both inner joins between the tables produce the same result set, regardless of whether they are arranged on the left or the right. Three line combinations meet the join condition.

-   The result sets of the two left joins depends on the arrangement of the tables on the left and right side. Apart from the three lines already produced by the inner join, an empty line (with null values) is created on the right for each extra line on the left. The fact that the empty line is displayed on the left in the final result is due to the structure of the internal table in the INTO clause. Here, the columns of DEMO\_JOIN1 are always read from the left and those of DEMO\_JOIN2 are always read from the right. If \* was specified in the SELECT list, and CORRESPONDING FIELDS was not used in the INTO clause, the result would be arranged differently.
...
-   The result sets of both cross joins without a WHERE condition are identical to the result sets of the corresponding inner and outer joins whose ON condition is always true. It makes no difference which side the data sources are specified on in the cross join. The number of lines is determined by 4 x 3 = 12.

-   The result set of the cross join with a WHERE condition is identical to the result set of an inner join with a corresponding ON condition. Note that in a cross join, all lines are read first and then selected, whereas the inner join only reads the lines that meet the conditions.

See also the executable example [SQL Expressions, Coalesce Function](https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abensql_expr_coalesce_abexa.htm), in which the default initial values set for the null values in the outer join are replaced by a different value.