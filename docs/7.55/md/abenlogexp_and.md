...
The joining of multiple logical expressions [log\_exp](javascript:call_link\('abenlogexp.htm'\)) using AND creates a new logical expression that is true if all logical expressions log\_exp are true. If one of the logical expressions is false, the join is also false.

Hints

-   The operator AND is a weaker join than NOT and stronger than OR and EQUIV.

-   The operator AND implements an AND join. Using [NOT](javascript:call_link\('abenlogexp_not.htm'\)) to negate such a join is the same as using a NAND join (Not AND). It is true if at least one expression is false.
