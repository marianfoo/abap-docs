...
Filling of a [ranges table](javascript:call_link\('abenranges_table_glosry.htm'\) "Glossary Entry") in a [SELECT](javascript:call_link\('abapselect.htm'\)) loop and its use in a logical expression in the conditional operator [COND](javascript:call_link\('abenconditional_expression_cond.htm'\)).
