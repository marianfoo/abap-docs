...
This example demonstrates simple group level processing using [GROUP BY](javascript:call_link\('abaploop_at_itab_group_by.htm'\)).
...
This example works in the same way as the executable example for [group level processing](javascript:call_link\('abenint_table_at_abexa.htm'\)). Here, the [group key](javascript:call_link\('abengroup_key_glosry.htm'\) "Glossary Entry") is constructed explicitly using the value of the column col1 and, unlike in group level processing, does not depend on the structure of the table and its sorting.

The statement [SUM](javascript:call_link\('abapsum.htm'\)) is replaced by a functional method that replaces the implicit behavior of the statement with an explicit and self-definable function.
