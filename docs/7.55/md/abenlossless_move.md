...
A [lossless assignment](javascript:call_link\('abenlossless_assignment_glosry.htm'\) "Glossary Entry") is an assignment between incompatible data types that is used to check whether the [conversion](javascript:call_link\('abenconversion_glosry.htm'\) "Glossary Entry") is possible without losses. Lossless assignments can be performed using the
