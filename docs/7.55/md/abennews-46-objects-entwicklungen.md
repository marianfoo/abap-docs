...
ABAP Objects does not support an instance destructor that can be implemented in ABAP. For special cases, and for internal use only, there is a so called [C destructor](javascript:call_link\('abenc_destructor.htm'\)).
