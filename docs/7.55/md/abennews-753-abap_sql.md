...
The existing name [Open SQL](javascript:call_link\('abenopen_sql_old_glosry.htm'\) "Glossary Entry") has been changed to [ABAP SQL](javascript:call_link\('abenabap_sql_glosry.htm'\) "Glossary Entry"). This renaming reflects that some parts of ABAP SQL now only support certain database platforms, specifically SAP HANA database, and hence that it is no longer fully platform-independent.
