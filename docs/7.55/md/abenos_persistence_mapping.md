---
title: "OS - Object-Relational Mapping"
description: |
  Technically speaking, all objects in ABAP Objects (including persistent objects managed by the Persistence Service) exist only transiently in the internal session(https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abeninternal_session_glosry.htm 'Glossary Entry') of an ABAP program. Conver
version: "7.55"
category: "oop"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abenos_persistence_mapping.htm"
...
Technically speaking, all objects in ABAP Objects (including persistent objects managed by the Persistence Service) exist only transiently in the [internal session](https://help.sap.com/doc/abapdocu_755_index_htm/7.55/en-US/abeninternal_session_glosry.htm "Glossary Entry") of an ABAP program. Conversely, AS ABAP data exists persistently in the database. The persistence of objects is achieved by object-relational mapping between database tables and the attributes of persistent classes. The mapping of existing persistent classes is defined using the Mapping Assistant of Class Builder. The support of the Mapping Assistant is stopped and no new mappings should be created.
...
In this case, the mapping is performed to ABAP Dictionary structures, rather than to database tables or DDIC database views. The application developer therefore has to program the type of persistent data storage in the relevant methods of the class agent.
