...
Calls a [built-in function](javascript:call_link\('abenopen_sql_builtin_functions.htm'\)) in ABAP SQL. The result is the return value of the function with the associated dictionary type. As a rule, the arguments of the built-in functions cover one or more SQL expressions.
