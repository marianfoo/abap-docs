...
In serializations, the entire ABAP data object that is bound to the current node, along with all its components, is recursively transformed to [asXML](javascript:call_link\('abenabap_xslt_asxml.htm'\)) format and used in the XML data in place of the statement.
