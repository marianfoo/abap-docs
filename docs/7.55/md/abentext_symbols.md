...
A text symbol behaves like a [constant](javascript:call_link\('abenconstant_glosry.htm'\) "Glossary Entry") and can be specified in [read positions](javascript:call_link\('abenreading_position_glosry.htm'\) "Glossary Entry") using its names as follows:
