...
-   For data objects whose data types are defined in reference to ABAP Dictionary, a different output length can be specified in the corresponding [domains](javascript:call_link\('abenddic_domains.htm'\)) or using the [built-in type](javascript:call_link\('abenddic_builtin_types.htm'\)). The output length specified here is used instead of the implicit output length from the table above.
