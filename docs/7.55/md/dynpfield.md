...
If a switch is assigned to the dynpro field dynp\_field in [Screen Painter](javascript:call_link\('abenswitch_glosry.htm'\) "Glossary Entry"), the statement FIELD is respected only if the dynpro field has been activated by the switch.
...
[dynpro - Handling of Messages from the PAI Event](javascript:call_link\('abendynp_field_messages.htm'\))
