...
In the context of the draft action [edit](javascript:call_link\('abenbdl_draft_action.htm'\)), %param contains the component preserve\_changes. It is a Boolean value and if it set to true, the draft action request is rejected if a draft already exists. If it is set to false, which is the default setting, the edit is executed, i. e. the active instance is copied to the draft database table and the existing draft is overwritten with the values of the active data. In that case, current changes on the existing draft instance are lost.
