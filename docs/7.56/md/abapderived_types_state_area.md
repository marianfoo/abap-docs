...
%state\_area is a component of [BDEF derived types](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenrap_derived_type_glosry.htm "Glossary Entry"). It is a string and used as a component of the [reported](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abaptype_response_for.htm) response parameter and filled by the [RAP BO provider](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenrap_bo_provider_glosry.htm "Glossary Entry").

If this component is filled, the framework interprets a message as [state message](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenrap_state_mes_glosry.htm "Glossary Entry"). Otherwise, a message is interpreted as a [transition message](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenrap_trans_mes_glosry.htm "Glossary Entry").

The %state\_area component is mainly relevant for the RAP BO provider in [draft](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenbdl_with_draft.htm) scenarios since state messages are persisted until the state of the RAP BO is changed. The string provided is not relevant for the [RAP BO consumer](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenrap_bo_consumer_glosry.htm "Glossary Entry"). It is only relevant for the RAP BO consumer to know whether the message is a transition message or not.

Note: You must manually invalidate state messages, i. e. remove persisted messages, in the RAP BO provider implementation. To identify the messages, use the respective key ([%tky](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abapderived_types_tky.htm)) and %state\_area string. For example:
...
In ABAP EML, state messages can only be accessed via an ABAP EML [READ](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abapread_entity_short.htm) statement. In all other ABAP EML statements, the [reported](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abaptype_response_for.htm) response parameter only contains transition messages.
...
The example [Example for message-related BDEF derived type components](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenderived_types_reported_abexa.htm) demonstrates the use of various BDEF derived type components including %state\_area with a managed RAP BO.