...
The %state\_area component is mainly relevant for the RAP BO provider in [draft](javascript:call_link\('abenbdl_with_draft.htm'\)) scenarios since state messages are persisted until the state of the RAP BO is changed. The string provided is not relevant for the [RAP BO consumer](javascript:call_link\('abenrap_bo_consumer_glosry.htm'\) "Glossary Entry"). It is only relevant for the RAP BO consumer to know whether the message is a transition message or not.
...
The following source code section taken from the CCIMP include of the global class of the [behavior pool](javascript:call_link\('abenbehavior_pool_glosry.htm'\) "Glossary Entry") BP\_DEMO\_MANAGED\_ROOT\_REPORTED demonstrates the component %state\_area in the context of a method implementation for a [validation](javascript:call_link\('abenbdl_validations.htm'\)) that takes care of checking field values of RAP BO instances before saving. See the executable program for this example further down.
