...
This statement appends the current content of the fields that were previously included in the field group header or field\_group using the statement [INSERT](javascript:call_link\('abapinsert_fg.htm'\)) to the [extract dataset](javascript:call_link\('abenextract_dataset_glosry.htm'\) "Glossary Entry") of the program. If no field group is specified after EXTRACT, the field group header is added implicitly.
