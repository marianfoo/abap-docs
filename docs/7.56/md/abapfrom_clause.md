...
-   An alias name tabalias prevents the original name [data\_source](javascript:call_link\('abapselect_data_source.htm'\)) from being used and this also applies to dynamic tokens in principle. This is not checked, however, until the introduction of the [strict modes](javascript:call_link\('abenabap_sql_strict_modes.htm'\)) in the syntax check from Release 7.40, SP08 and may raise an exception.
-   If an alias name is to be named exactly like an addition of the SELECT statement exactly, it may be necessary to prefix it with the [escape character !](javascript:call_link\('abennames_escaping.htm'\)).
