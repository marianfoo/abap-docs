...
When inserting lines as a block into an internal table with non-unique [primary table keys](javascript:call_link\('abenprimary_table_key_glosry.htm'\) "Glossary Entry"), the order of the duplicate lines in relation to this primary key is preserved. This does not apply for [secondary keys](javascript:call_link\('abensecondary_key_glosry.htm'\) "Glossary Entry").
