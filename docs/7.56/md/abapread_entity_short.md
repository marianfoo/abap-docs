...
The short form of the READ statement is used for read [operations](javascript:call_link\('abapread_entity_entities_op.htm'\)) from a single RAP BO entity only. The entity name (bdef) can be specified directly, that is, reading from a child entity can also be done directly and not via the [root entity](javascript:call_link\('abenroot_entity_glosry.htm'\) "Glossary Entry") of a RAP BO. In this case, bdef must be the full entity name and cannot be an alias name because the context of a RAP BO is unknown. It is possible to include all three read options in one operation per entity bdef: [read operation](javascript:call_link\('abenrap_read_operation_glosry.htm'\) "Glossary Entry"), [read-by-association operation](javascript:call_link\('abenrap_rba_operation_glosry.htm'\) "Glossary Entry") and an operation executing a function, but only one of each kind within the same READ ENTITY statement.
