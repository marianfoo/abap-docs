...
The additions PROGRAM and DIRECTORY ENTRY are used to set the attributes for the syntax check. The addition WITH CURRENT SWITCHSTATES affects which switch configuration is used for the syntax check. The other [error\_handling](javascript:call_link\('abapsyntax-check_error_handling.htm'\)) additions can be used to identify other attributes of the first syntax error.
