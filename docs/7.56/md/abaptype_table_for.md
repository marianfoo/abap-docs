...
Special ABAP type that is used for internal tables in the context of [RAP](javascript:call_link\('abenrap_glosry.htm'\) "Glossary Entry") to enable the communication between a [RAP BO provider](javascript:call_link\('abenrap_bo_provider_glosry.htm'\) "Glossary Entry") and a [RAP BO consumer](javascript:call_link\('abenrap_bo_consumer_glosry.htm'\) "Glossary Entry"). These internal tables hold input parameters, or output parameters respectively, to enable mass processing of data by bundling ABAP EML READ or MODIFY requests in one method call.
...
The internal tables contain various [special components](javascript:call_link\('abapderived_types_comp.htm'\)), also depending on the BDEF.
...
For details on the components, see [Components of BDEF Derived Types](javascript:call_link\('abapderived_types_comp.htm'\)). For details on the BDEF specifications, see the subtopics in [ABAP CDS - BDL for Behavior Definitions](javascript:call_link\('abenbdl.htm'\)).
