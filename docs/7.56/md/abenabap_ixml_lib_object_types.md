...
All object types in iXML Library include the interface IF\_IXML\_UNKNOWN. Reference variables of this type can point to all objects in iXML Library. The interface IF\_IXML\_UNKNOWN contains the method QUERY\_INTERFACE, which can be used for special [down casts](javascript:call_link\('abenixml_down_casts.htm'\)) within iXML Library.
