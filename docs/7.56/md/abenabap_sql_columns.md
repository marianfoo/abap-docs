...
Specifies an element of a data source of a [CDS view entity](javascript:call_link\('abencds_data_source_v2.htm'\)) or of a [CDS DDIC-based view](javascript:call_link\('abencds_data_source_v1.htm'\)) after a path expression [sql\_path](javascript:call_link\('abenabap_sql_path.htm'\)). This can be specified in SELECT statements where path expressions can be used in all places where a column can be specified, unless otherwise indicated.
