...
The strict mode of the syntax check in Release 7.51 applies to all ABAP SQL statements that use one of the following attributes introduced in [Release 7.51](javascript:call_link\('abennews-751-abap_sql.htm'\)):
...
The strict mode in Release 7.51 covers all rules of the [strict mode in Release 7.50](javascript:call_link\('abenabap_sql_strictmode_750.htm'\)), plus the following rules:
...
-   In the statement [OPEN CURSOR](javascript:call_link\('abapopen_cursor.htm'\)), the additions grouped under [UP TO, OFFSET](javascript:call_link\('abapselect_up_to_offset.htm'\)), and [abap\_options](javascript:call_link\('abapselect_additions.htm'\)) must also be specified at the end of the SELECT statement.
