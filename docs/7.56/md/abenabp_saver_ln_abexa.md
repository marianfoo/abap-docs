...
define behavior for demo\_umanaged\_child\_late\_num2 alias child
...
    SELECT saver\_method, pid, key\_field, fail\_cause, msg
      FROM demo\_tab\_log\_ln
      INTO TABLE @DATA(saver\_log).
    LOOP AT saver\_log TRANSPORTING NO FIELDS WHERE key\_field NE 0.
      EXIT.
    ENDLOOP.
    IF sy-subrc EQ 0.
      out->write\_html(
...
      DELETE FROM demo\_tab\_log\_ln.
    ELSE.
      SELECT saver\_method, pid, fail\_cause, msg
        FROM demo\_tab\_log\_ln
        INTO TABLE @DATA(saver\_log\_wo\_key).
      out->write\_html(
            '<b>b) Saver log</b>'
            )->write( saver\_log\_wo\_key ).
      CLEAR saver\_log\_wo\_key.
      DELETE FROM demo\_tab\_log\_ln.
    ENDIF.
  ENDMETHOD.
  METHOD get\_db\_entries.
    SELECT key\_field, field1, field2, field3, field4
    FROM demo\_tab\_root\_3 INTO TABLE @DATA(db\_tab)
    ORDER BY key\_field.
