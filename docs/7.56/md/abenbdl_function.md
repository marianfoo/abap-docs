...
If you try to modify data in the function implementation in the ABAP behavior pool, there's no error message, but a runtime error occurs when a [RAP BO consumer](javascript:call_link\('abenrap_bo_consumer_glosry.htm'\) "Glossary Entry") tries to access the function in question.
