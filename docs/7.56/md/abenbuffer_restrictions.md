...
The fact that certain operations can only be performed on the database and not in the table buffer produces the further restrictions for reads listed below. If these restrictions are not respected, [read ABAP SQL statements](javascript:call_link\('abenabap_sql_reading.htm'\)) bypass the [table buffer](javascript:call_link\('abentable_buffer_glosry.htm'\) "Glossary Entry") implicitly and access the DDIC database tables or DDIC views directly.
