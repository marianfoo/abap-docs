...
An input parameter called pname can be used as an operand in the following places of the [DEFINE HIERARCHY](javascript:call_link\('abencds_f1_define_hierarchy.htm'\)) statement using the same syntax [:pname](javascript:call_link\('abencds_parameter_v1.htm'\)) or [$parameters.pname](javascript:call_link\('abencds_parameter_v1.htm'\)) as in CDS DDIC-based views:
