...
Defines an element of a [CDS DDIC-based view](javascript:call_link\('abencds_v1_view_glosry.htm'\) "Glossary Entry") . The elements can be specified as follows:
...
These names are also the names of the view fields of the [CDS-managed DDIC view](javascript:call_link\('abencds_mngdddic_view_glosry.htm'\) "Glossary Entry") and the corresponding [rules](javascript:call_link\('abenddic_database_views.htm'\)) that must be kept.
