...
Like a [CDS view](javascript:call_link\('abencds_view_glosry.htm'\) "Glossary Entry"), a CDS table function is client-dependent by default and [implicit client handling](javascript:call_link\('abenabap_sql_client_handling.htm'\)) is applied in ABAP SQL reads.
