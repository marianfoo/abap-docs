...
The following [CDS annotation](javascript:call_link\('abencds_annotation_glosry.htm'\) "Glossary Entry") can define [table buffering](javascript:call_link\('abentable_buffering_glosry.htm'\) "Glossary Entry") for a CDS view entity:

-   @AbapCatalog.entityBuffer defines whether [table buffering](javascript:call_link\('abensap_puffering.htm'\)) is allowed.
...
-   [@AbapCatalog.buffering.status](javascript:call_link\('abencds_view_anno_v1.htm'\)), [@AbapCatalog.buffering.type](javascript:call_link\('abencds_view_anno_v1.htm'\)), and [@AbapCatalog.buffering.numberOfKeyFields](javascript:call_link\('abencds_view_anno_v1.htm'\)) are not supported in CDS view entities. They are only valid for CDS DDIC-based views.
