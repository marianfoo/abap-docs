...
description: |
  This example demonstrates all standard operations(https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abencrud_glosry.htm 'Glossary Entry') to modify and read from a simple unmanaged RAP BO(https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenrap_bo_glosry.htm 'Glossary Entry').
version: "7.56"
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abeneml_modify_op_u_abexa.htm"
...
This example demonstrates all [standard operations](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abencrud_glosry.htm "Glossary Entry") to modify and read from a simple unmanaged [RAP BO](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenrap_bo_glosry.htm "Glossary Entry"). The following operations are covered:
...
In an unmanaged RAP scenario - as opposed to managed RAP scenarios - the developer must take care of the implementation of the standard operations. This simplified example is not a real life scenario and rather focuses on the technical side by showing how the methods of an [ABAP behavior pool (ABP)](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenbehavior_pool_glosry.htm "Glossary Entry") might be self-implemented taking the handling of the transactional buffer in a managed scenario as a role model in a simplified way. This implementation can, for example, be called using [ABAP EML](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abeneml_glosry.htm "Glossary Entry") outside of the behavior pool.

The transactional buffer of the unmanaged RAP BO in the example is constituted by internal tables for the sake of having a self-contained example reducing the complexity. The tables act as a memory area into which data from the [persistent database tables](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenrap_persistent_table_glosry.htm "Glossary Entry") are read and from which data is saved back to the database tables after triggering the saving of modified instances (with a COMMIT ENTITIES statement).

Note

The code is meant to give an idea about the principles of an unmanaged RAP scenario, the fact that everything must be self-implemented and the significance of the transactional buffer that can be accessed by [ABAP EML](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abeneml_glosry.htm "Glossary Entry"), and should therefore not be reused in a productive scenario.
...
The [CDS behavior definition](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abencds_behavior_definition_glosry.htm "Glossary Entry") DEMO\_UNMANAGED\_ROOT\_2 is defined in [CDS BDL](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abencds_bdl_glosry.htm "Glossary Entry") as follows:
...
        FAILED   failed
        REPORTED reported.
    IF failed-demo\_unmanaged\_root\_2 IS NOT INITIAL
    AND reported-demo\_unmanaged\_root\_2 IS NOT INITIAL.
      out->write\_html( '<b>Responses for the operations</b>' ).
      display\_responses(  ).
    ELSE.
      out->write\_text(
     \`No FAILED and REPORTED responses for the operations.\`
               ).
    ENDIF.
    COMMIT ENTITIES.
    SELECT \* FROM demo\_tab\_root\_3 INTO TABLE @DATA(create)
      ORDER BY key\_field.
    SELECT \* FROM demo\_tab\_child\_3 INTO TABLE @DATA(create\_by\_assoc)
      ORDER BY key\_field.
    out->write\_html(
        '<b>DB table entries after the operations</b>'
       )->write( create
       )->write\_html(
       '<u><b><i>Create-by-association operations</i></b></u>'
       )->write\_html(
          '<b>DB table entries after the operations</b>'
...
    ENDIF.
    COMMIT ENTITIES.
    SELECT \* FROM demo\_tab\_root\_3 INTO TABLE @DATA(update)
      ORDER BY key\_field.
    out->write\_html(
      '<b>DB table entries after the operations</b>'
            )->write( update ).
...
    ENDIF.
    COMMIT ENTITIES.
    SELECT \* FROM demo\_tab\_root\_3 INTO TABLE @DATA(delete)
      ORDER BY key\_field.
    out->write\_html(
        '<b>DB table entries after the operations</b>'
                      )->write( delete ).
...
      \`No FAILED and REPORTED responses for the operations.\` ).
    ENDIF.
    out->write\_html( '<b>Result of the operations</b>'
                   )->write( read\_result
                   )->write\_html(
          '<u><b><i>Read-by-association operations</i></b></u>' ).
    READ ENTITY demo\_unmanaged\_root\_2
...
          %control-field4 = if\_abap\_behv=>mk-on )
        )
         RESULT DATA(read\_by\_assoc)
         FAILED failed
         REPORTED reported.
    IF failed-demo\_unmanaged\_root\_2 IS NOT INITIAL
    AND reported-demo\_unmanaged\_root\_2 IS NOT INITIAL.
      out->write\_html(
          '<b>Responses for the operations</b>'
                ).
      display\_responses(  ).
    ELSE.
      out->write\_text(
...
    LOOP AT failed-demo\_unmanaged\_root\_2
...
    LOOP AT reported-demo\_unmanaged\_root\_2
