...
title: "Syntax"
description: |
  ... escape( val = text(https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenstring_functions_val.htm) format = format ) ... Effect This built-in function(https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenbuilt_in_functions.htm) returns the content of the character string i
version: "7.56"
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenescape_functions.htm"
...
... escape( [val = text](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenstring_functions_val.htm) format = format ) ...

Effect

This [built-in function](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenbuilt_in_functions.htm) returns the content of the character string in [text](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenstring_functions_val.htm), and escapes certain special characters with escape characters according to a rule specified in format.
...
This function can be specified in [general](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abengeneral_expr_position_glosry.htm "Glossary Entry") and [character-like expression positions](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abencharlike_expr_position_glosry.htm "Glossary Entry"). The return value has the type string.
...
[String Functions, escape for HTML](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenstring_function_escape_abexa.htm)
...
All characters with codes from x80 are converted to their [UTF-8](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenutf8_glosry.htm "Glossary Entry") representation. Depending on the character, one to four bytes are represented in the form %hh, where hh is the hexadecimal value of a byte.
...
The program DEMO\_ESCAPE\_XSS demonstrates the replacement rules of the formats E\_XSS\_... that make it possible to prevent attacks using Cross Site Scripting ([XSS](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenxss_glosry.htm "Glossary Entry")) on Web applications. There are rules for XML/HTML content, JavaScript content, Cascading Style Sheets (CSS), and URL content.
...
-   URLs and URIs: Format E\_XSS\_URL. All characters except \[0-9\], \[a-z\], \[A-Z\], \*, \-, ., and \_ are transformed to %hh, where hh is the hexadecimal value of the code. All characters with codes from x80 are converted to their [UTF-8](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenutf8_glosry.htm "Glossary Entry") representation. Depending on the character, one to four bytes are represented in the form %hh, where hh is the hexadecimal value of a byte.
...
[String Functions, escape for XSS](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenstring_function_esc_xss_abexa.htm)
...
-   Cause: A character cannot be converted in a conversion to UTF-8. This can only occur with characters from the [surrogate area](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abensurrogate_area_glosry.htm "Glossary Entry"). The position and code of the character is specified in the exception object.
