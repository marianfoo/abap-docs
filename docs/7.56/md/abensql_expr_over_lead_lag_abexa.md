...
title: "Source Code"
description: |
  REPORT demo_select_over_lead_lag. CLASS demo DEFINITION. PUBLIC SECTION. CLASS-METHODS: main, class_constructor. ENDCLASS. CLASS demo IMPLEMENTATION. METHOD main. DATA(out) = cl_demo_output=>new( ). SELECT char1, char2, num1, ROW_NUMBER( ) OVER( PARTITION BY char1 ORDER BY char2 ) AS rnum, L
version: "7.56"
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abensql_expr_over_lead_lag_abexa.htm"
...
This example demonstrates [window expressions](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abapselect_over.htm) with the [window functions](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenwindow_function_glosry.htm "Glossary Entry") [LEAD](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abensql_win_func.htm) and [LAG](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abensql_win_func.htm).
...
           INTO TABLE @DATA(windowed\_order\_ascending) ##no\_text.
...
           FROM demo\_expressions
           ORDER BY char1, char2 "#EC CI\_NOWHERE
           INTO TABLE @DATA(windowed\_order\_descending) ##no\_text.
...
The example shows results of the window functions [LEAD](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abensql_win_func.htm) and [LAG](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abensql_win_func.htm) to which all three possible arguments are passed. Like in the executable example, the windows are determined for [window expressions with sorting](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abensql_expr_over_order_by_abexa.htm).
