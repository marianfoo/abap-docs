...
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenst_tt_ref.htm"
...
Exactly one node of the tree structures of the current template can be set as the current node at any position within a Simple Transformation (statically and when the program is executed). The current node hides the data roots in the sense that a reference name does not refer to a root name, but to the component name of the current node (see [Addressing Subnodes](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenst_addressing_subnodes.htm)). If the current node is defined, it is the implicit operand of many [ST statements](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenst_statements.htm), but it can also be addressed explicitly using $ref.

This section shows how to set the current node explicitly using *\[*tt:*\]*ref. In addition, the statement [tt:loop](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenst_tt_loop.htm) for transforming internal tables also sets the current node.
...
As a result of the [addressing rules](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenst_addressing_nodes.htm), only a subnode of the current node can be set as the new current node, unless a data root is being set.
...
In a [literal XML element](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenst_xml_literals.htm), tt:ref is a special attribute in the namespace of the ST statement and sets the current node there. The same rules apply to node as when setting using the statement tt:ref. The currently set node is valid only within the context of the XML element.
...
Also see the example for calling a transformation in the [tt:value, Structures](https://help.sap.com/doc/abapdocu_756_index_htm/7.56/en-US/abenst_tt_value_structure.htm) section.