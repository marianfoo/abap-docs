...
-   It depends on the database platform and different settings, how the contents of the internal table is passed to the database and where it is transformed into conditions. If [FDA](javascript:call_link\('abenfast_data_access_glosry.htm'\) "Glossary Entry") write access is available, it can be used by FOR ALL ENTRIES.
