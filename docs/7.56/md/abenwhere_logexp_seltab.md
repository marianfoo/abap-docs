...
This variant of the operator IN checks whether the operands on the left side match a [ranges condition](javascript:call_link\('abenranges_condition_glosry.htm'\) "Glossary Entry") in a [ranges table](javascript:call_link\('abenranges_table_glosry.htm'\) "Glossary Entry"). The relational expression is true if the value of the operand operand is (not) in the result set described in the rows of the [ranges table](javascript:call_link\('abenranges_table_glosry.htm'\) "Glossary Entry") range\_tab specified as a [host variable](javascript:call_link\('abenabap_sql_host_variables.htm'\)).
