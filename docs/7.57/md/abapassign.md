...
ASSIGN [mem\_area](javascript:call_link\('abapassign_mem_area.htm'\)) TO <fs> [casting\_spec](javascript:call_link\('abapassign_casting.htm'\))
                        [range\_spec](javascript:call_link\('abapassign_range.htm'\))
...
If the assignment is not successful. the behavior depends on the assigned [mem\_area](javascript:call_link\('abapassign_mem_area.htm'\)) and on the addition [ELSE UNASSIGN](javascript:call_link\('abapassign_else_unassign.htm'\)):

-   If a [static assignment](javascript:call_link\('abapassign_mem_area_static_dobj.htm'\)) is not successful, sy-subrc is not set. No memory area is assigned to the field symbol. The field symbol has the state unassigned after the ASSIGN statement. The addition ELSE UNASSIGN is used implicitly and must not be specified.
-   If a [dynamic assignment](javascript:call_link\('abapassign_mem_area_dynamic_dobj.htm'\)), an [assignment of dynamic components](javascript:call_link\('abapassign_dynamic_components.htm'\)), a [dynamic access](javascript:call_link\('abapassign_mem_area_dynamic_access.htm'\)) or an [assignment of a table expression](abapassign_mem_area_writable_exp.htm#!ABAP_ALTERNATIVE_3@3@) is not successful, sy-subrc is set to 4 or 8 and:
    ...
    Some invalid dynamic specifications in the [assignment of dynamic components](javascript:call_link\('abapassign_dynamic_components.htm'\)) do not set sy-subrc but raise an exception.
...
[ASSIGN, ELSE UNASSIGN](javascript:call_link\('abapassign_else_unassign.htm'\))