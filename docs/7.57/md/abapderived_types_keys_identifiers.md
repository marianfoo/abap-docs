...
-   RAP interaction phase
    ...
    -   [%tky](javascript:call_link\('abapderived_types_tky.htm'\)): The component group includes %key and %is\_draft, hence, it is predominantly used in draft scenarios; but it is recommended that %tky is used also in non-draft scenarios for the keys so as to be prepared for a potential later switch to draft
...
-   RAP interaction phase
    ...
    -   %key: might be used to include preliminary keys as RAP BO instance identifiers; it must be ensured that the [adjust\_numbers](javascript:call_link\('abensaver_adjust_numbers.htm'\)) method in the RAP save sequence maps the final key values to %key
