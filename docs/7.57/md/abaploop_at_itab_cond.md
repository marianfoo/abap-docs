...
These optional additions to [LOOP AT itab](javascript:call_link\('abaploop_at_itab.htm'\)) specify a subset of lines to be processed and the order in which the loop is executed.
...
-   The processing order can be defined with:
    -   USING KEY [keyname](javascript:call_link\('abenkeyname.htm'\)) that defines the table key that is used for the loop and thus overrides the standard order determined by the table category.
    -   STEP that can define a reverse order by specifying a negative step size.

If none of the conditions are specified, all table lines are read in the standard order that is defined by the table category as described under [LOOP AT itab](javascript:call_link\('abaploop_at_itab.htm'\)).
...
The optional addition USING KEY specifies a table key [keyname](javascript:call_link\('abenkeyname.htm'\)) with which the processing is executed. The specified table key affects the order in which the table lines are accessed, and the evaluation of the other conditions.
...
-   [Specification of a hash key](javascript:call_link\('abenhash_key_glosry.htm'\) "Glossary Entry")
    ...
    The lines are processed in the order in which they were inserted into the table. In each loop pass, the system field sy-tabix contains the value 0. A preceding sort using the statement [SORT](javascript:call_link\('abapsort_itab.htm'\)) does not change this processing order.
...
-   If a secondary table key is specified, a simultaneously specified WHERE condition also must be [optimizable](javascript:call_link\('abenitab_where_optimization.htm'\)), otherwise a syntax error occurs or an exception is raised.
...
-   The control structures for [group level processing](javascript:call_link\('abengroup_level_processing_glosry.htm'\) "Glossary Entry") [AT FIRST, AT LAST, AT NEW, and AT END OF](javascript:call_link\('abapat_itab.htm'\)) are not supported with STEP.
...
[LOOP with Step Size](javascript:call_link\('abenloop_at_itab_step_abexa.htm'\))
...
![Example](exa.gif "Example") [itab - Loop with Step Size](javascript:call_link\('abenloop_at_itab_step_abexa.htm'\))
![Example](exa.gif "Example") [itab - STEP Examples](javascript:call_link\('abenloop_step_examples_abexa.htm'\))