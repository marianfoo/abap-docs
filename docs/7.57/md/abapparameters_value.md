...
-   If a program is not loaded into an [internal session](javascript:call_link\('abeninternal_session_glosry.htm'\) "Glossary Entry") using a SUBMIT call, but using a [dialog](javascript:call_link\('abendialog_transaction_glosry.htm'\) "Glossary Entry") or an [object transaction](javascript:call_link\('abenoo_transaction_glosry.htm'\) "Glossary Entry"), or in an external procedure call instead, all the values val specified using DEFAULT are passed to the associated parameters para when any selection screen is called for the first time using [CALL SELECTION-SCREEN](javascript:call_link\('abapcall_selection_screen.htm'\)) before the event [AT SELECTION-SCREEN OUTPUT](javascript:call_link\('abapat_selection-screen.htm'\)).
