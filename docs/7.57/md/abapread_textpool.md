...
[Short Reference](javascript:call_link\('abapread_textpool_shortref.htm'\))
...
-   When handling [global classes](javascript:call_link\('abenglobal_class_glosry.htm'\) "Glossary Entry") and [function pools](javascript:call_link\('abenfunction_pool_glosry.htm'\) "Glossary Entry"), it should be noted that the name of the master program must be specified in the repository and not the name of the class or function pool. The composition of these names is documented in the statements [CLASS-POOL](javascript:call_link\('abapclass-pool.htm'\)) and [FUNCTION-POOL](javascript:call_link\('abapfunction-pool.htm'\)).
