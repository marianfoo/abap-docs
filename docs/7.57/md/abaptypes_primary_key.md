...
An alias name alias\_name can be defined for the primary key in sorted tables and hashed tables, for as long as the primary key is not generic. The alias name is in the namespace of the secondary key, must comply with the [naming conventions](javascript:call_link\('abennaming_conventions.htm'\)), and must be unique. It cannot be one of the predefined names primary\_key or loop\_key. It enables the primary key to be addressed like a secondary key by means of a self-defined name. The syntax requires the name primary\_key to be specified explicitly when defining an alias name.
