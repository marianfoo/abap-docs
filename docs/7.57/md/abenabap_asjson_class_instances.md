...
To transform classes to [JSON](javascript:call_link\('abenjson_glosry.htm'\) "Glossary Entry") using the statement [CALL TRANSFORMATION](javascript:call_link\('abapcall_transformation.htm'\)) or to create classes from JSON data, their classes must implement the interface IF\_SERIALIZABLE\_OBJECT. The instance of a class (object) is represented as an object component of the object %heap as follows:
...
The [tag interface](javascript:call_link\('abentag_interface_glosry.htm'\) "Glossary Entry") IF\_SERIALIZABLE\_OBJECT has the same meaning for JSON as for [asXML](javascript:call_link\('abenasxml_class_instances.htm'\)). This means it defines which attributes of a class are serialized. In addition, helper methods can be implemented that adjust behavior (see the [example](javascript:call_link\('abenserializable_object_abexa.htm'\))).
