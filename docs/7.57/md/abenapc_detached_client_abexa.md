...
-   The connection handle obtained in the method on\_open of the handler class can either be displayed or used immediately in the program to create an attached client for the detached client.
    -   A connection handle in the output can be copied and used in the program of the executable [attached client](javascript:call_link\('abenapc_attached_client_abexa.htm'\)) example.
...
A breakpoint in the method ON\_MESSAGE of the APC handler class CL\_APC\_WSP\_EXT\_DEMO\_APC\_PCP can be used to monitor the arrival and processing of the message. The reaction to custom messages is prevented here. If the message is sent using [AMC](javascript:call_link\('abenamc.htm'\)), the response from the server is received by all associated AMC receivers. The attached client cannot itself wait for the response.