...
An extension own authorization context may be defined before, between, or after the other [components of an extension BDEF](javascript:call_link\('abenbdl_extensibility_managed_unm.htm'\)). It is also possible to define an empty own authorization context.

For details on the concept of an own authorization context, see topic [CDS BDL - define own authorization context](javascript:call_link\('abenbdl_def_own_auth_context.htm'\)).
