...
Field characteristics that require an implementation (currently, this is the case only for features:instance) cannot be newly added for regular fields, but for augmented [virtual elements](javascript:call_link\('abencds_virtual_element_glosry.htm'\) "Glossary Entry").
...
Topic [CDS BDL - field characteristics](javascript:call_link\('abenbdl_field_char.htm'\)) describes how they work.
