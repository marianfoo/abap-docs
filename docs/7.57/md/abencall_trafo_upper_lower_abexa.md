...
-   Deserializations of the transformed asXML data with the self-written XSL transformations, on the other hand, are successful.
    ...
    -   DEMO\_ID\_FROM\_TO\_MIXED also calls a method for [to\_mixed](javascript:call_link\('abencase_functions.htm'\)). This simple example transformation is not, however, symmetrical in all cases.
