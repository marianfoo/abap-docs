...
The program DEMO\_CDS\_AUTH\_LIT\_PFCG\_VE uses [SELECT](javascript:call_link\('abapselect.htm'\)) to access the view entity. CDS access control restricts the selections to data for which the current user has authorization according to classic role maintenance (transaction transaction PFCG) and that meets the literal condition.
