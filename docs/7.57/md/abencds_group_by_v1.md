...
             [path\_expr1](javascript:call_link\('abencds_case_expression_v1.htm'\)), [path\_expr2](javascript:call_link\('abencds_case_expression_v1.htm'\)), ...

Effect

Groups those rows in the result set of a [CDS DDIC-based view (obsolete)](javascript:call_link\('abencds_v1_view_glosry.htm'\) "Glossary Entry") that have the same content in the elements specified by the fields field1, field2, ... or [path expressions](javascript:call_link\('abencds_case_expression_v1.htm'\)) path\_expr1, path\_expr2 ... as a single row. The fields must be specified using the same names as the fields in the data source [data\_source](javascript:call_link\('abencds_data_source_v1.htm'\)). It is not allowed to use the alias names defined in the current CDS view using with [AS](javascript:call_link\('abencds_select_list_entry_v1.htm'\)).
...
A [WHERE condition](javascript:call_link\('abencds_where_clause_v1.htm'\)) is evaluated before the rows are combined using GROUP BY.
