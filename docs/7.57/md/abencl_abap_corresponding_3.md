...
For the parameters source and destination, the same applies as to the method [CREATE](javascript:call_link\('abencl_abap_corresponding_1.htm'\)). An internal table of the type CL\_ABAP\_CORRESPONDING=>MAPPING\_TABLE\_VALUE that contains the mapping rule, must be passed to the parameter mapping. This mapping table has the same components, with the same meaning as a mapping table of type [CL\_ABAP\_CORRESPONDING=>MAPPING\_TABLE](javascript:call_link\('abencl_abap_corresponding_1.htm'\)) with an additional column and further values for the column KIND:
