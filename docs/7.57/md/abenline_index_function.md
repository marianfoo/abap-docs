...
-   If a search key is specified without a table key being specified explicitly, the number of a found line is returned with respect to the [primary table index](javascript:call_link\('abenprimary_table_index_glosry.htm'\) "Glossary Entry") and, in the case of a [hashed table](javascript:call_link\('abenhashed_table_glosry.htm'\) "Glossary Entry"), the value \-1.
