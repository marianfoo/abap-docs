...
[Classic lists](javascript:call_link\('abenabap_dynpro_list.htm'\)) should no longer be used in production application programs. SAP List Viewer (ALV) should be used instead of classic lists. If classic lists are still used for any reason, then the following constructs must be avoided:
