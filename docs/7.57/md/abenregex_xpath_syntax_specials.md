...
The following tables summarize the special characters in [XPath regular expressions](javascript:call_link\('abenxpath_regex_glosry.htm'\) "Glossary Entry").
...
The syntax of replacement patterns for XPath regular expressions is the same as for [PCRE regular expressions](javascript:call_link\('abenregex_pcre_syntax_specials.htm'\)).