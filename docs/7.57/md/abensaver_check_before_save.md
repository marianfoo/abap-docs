...
-   It is responsible for checking the consistency of the [transactional buffer](javascript:call_link\('abentransactional_buffer_glosry.htm'\) "Glossary Entry") before the [RAP late save phase](javascript:call_link\('abenlate_rap_save_phase_glosry.htm'\) "Glossary Entry"). To guarantee a successful saving for all involved [RAP BOs](javascript:call_link\('abenrap_bo_glosry.htm'\) "Glossary Entry"), [validations](javascript:call_link\('abenbdl_validations.htm'\)) should be implemented that are called within the check\_before\_save method.
-   If the method returns an error in the failed parameter, the save sequence is terminated and the [cleanup\_finalize](javascript:call_link\('abapsaver_class_cleanup_finalize.htm'\)) method is called to discard all changes made to the data in the current [RAP LUW](javascript:call_link\('abenrap_luw_glosry.htm'\) "Glossary Entry") and to clean up the transactional buffer.
