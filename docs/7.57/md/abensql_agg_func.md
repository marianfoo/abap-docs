...
If the aggregate function CORR is used as [window function](javascript:call_link\('abensql_win_func.htm'\)), the ROWS BETWEEN addition is only supported with UNBOUNDED PRECEDING and UNBOUNDED FOLLOWING. Otherwise, a syntax warning occurs, if detected at compile time, or a catchable exception of class CX\_SY\_OPEN\_SQL\_DB occurs, if detected at runtime.
...
If the aggregate function CORR\_SPEARMAN is used as [window function](javascript:call_link\('abensql_win_func.htm'\)), the ROWS BETWEEN addition is only supported with UNBOUNDED PRECEDING and UNBOUNDED FOLLOWING. Otherwise, a syntax warning occurs, if detected at compile time, or a catchable exception of class CX\_SY\_OPEN\_SQL\_DB occurs, if detected at runtime.
