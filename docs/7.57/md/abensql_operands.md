...
[Typed literals](javascript:call_link\('abenabap_sql_typed_literals.htm'\)) are always handled as elementary SQL expressions, while [untyped literals](javascript:call_link\('abenabap_sql_untyped_literals.htm'\)) can also be handled as host variables, depending on the operand position.
