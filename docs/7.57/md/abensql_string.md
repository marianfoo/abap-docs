...
String expression in ABAP SQL. The operator && concatenates the character strings in two adjacent operands into a single character string. [Elementary SQL expressions](javascript:call_link\('abensql_elem.htm'\)) can be specified as the operands sql\_elem1, sql\_elem2 with the following restrictions:
