...
The use of these functions enforces the [strict mode from Release 7.53](javascript:call_link\('abenabap_sql_strictmode_753.htm'\)).
...
The actual parameter for the optional formal parameter user must have the built-in type [CHAR](javascript:call_link\('abenddic_builtin_types.htm'\)) with length 12. The default value is the user name of the current [ABAP user](javascript:call_link\('abenabap_user_glosry.htm'\) "Glossary Entry"). The same applies to the parameter client as to the function ABAP\_SYSTEM\_TIMEZONE.
