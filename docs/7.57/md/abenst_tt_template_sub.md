...
Like the [main template](javascript:call_link\('abenst_tt_template_main.htm'\)), subtemplates are defined with tt:template. The attribute name must be used to define a unique name tmpl for the subtemplate.

Data in Subtemplates   

In a subtemplate, custom data can be declared: tt:context can be used to define a context for the statements [tt:root](javascript:call_link\('abenst_tt_root.htm'\)), [tt:variable](javascript:call_link\('abenst_tt_variable.htm'\)), and [tt:parameter](javascript:call_link\('abenst_tt_parameter.htm'\)). The statement tt:context is allowed only in subtemplates and the statements tt:root, tt:variable, and tt:parameter are allowed in a template only within tt:context.
...
-   Parameters
    ...
    The parameters declared using tt:parameter are formal parameters of the subtemplate, to which a value can be passed using [tt:with-parameter](javascript:call_link\('abenst_tt_apply.htm'\)). Otherwise, the same applies as to the parameters of the main template.
