...
-   TYPE reference to an [enumerated type](javascript:call_link\('abenenum_type_glosry.htm'\) "Glossary Entry") [enum\_type](javascript:call_link\('abaptypes_enum.htm'\)) defined in ABAP, or to an enumerated type [EnumType](javascript:call_link\('abencds_define_enum_type.htm'\)) defined in ABAP CDS.
-   LIKE reference to a data object with [enumerated type](javascript:call_link\('abenenum_type_glosry.htm'\) "Glossary Entry"). The following options are possible:
