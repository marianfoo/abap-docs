...
For conn, a literal or a [host variable](javascript:call_link\('abapexec_host.htm'\)) that contains one of the following values can be specified:
...
Both of these instances are case-sensitive. If a secondary connection that is not in the database table DBCON is specified, a catchable exception of the class CX\_SY\_NATIVE\_SQL\_ERROR is raised. If a [secondary database](javascript:call_link\('abensecondary_db_glosry.htm'\) "Glossary Entry") cannot be accessed, sy-subrc is set to 4.
...
-   DEFAULT in uppercase letters or DEFAULT specified directly can be used to specify the [standard connection](javascript:call_link\('abenstandard_db_connection_glosry.htm'\) "Glossary Entry").
