...
-   It clears the [RAP transactional buffer](javascript:call_link\('abentransactional_buffer_glosry.htm'\) "Glossary Entry"). You can also undo changes if other RAP BOs are involved and there are dependencies on other RAP BOs in the early save phase.
-   It is only called if issues occur in the [finalize](javascript:call_link\('abensaver_finalize.htm'\)) or [check\_before\_save](javascript:call_link\('abensaver_check_before_save.htm'\)) methods within the current [RAP transaction](javascript:call_link\('abenrap_luw_glosry.htm'\) "Glossary Entry"). After calling the cleanup\_finalize method, the [RAP late save phase](javascript:call_link\('abenlate_rap_save_phase_glosry.htm'\) "Glossary Entry") is not processed. Instead, you return to the [RAP interaction phase](javascript:call_link\('abenrap_int_phase_glosry.htm'\) "Glossary Entry").
