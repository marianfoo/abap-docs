...
The length len must be specified directly as a positive number with a maximum of two digits. If a line is too long, it is truncated outside [blocks](javascript:call_link\('abapselection-screen_block.htm'\)) with frames at position 83 and at the right margin of the block inside a block with a frame. The position can be either a number pos between 1 and 83, POS\_LOW, or POS\_HIGH. POS\_LOW specifies the position where the input field for a [selection parameter](javascript:call_link\('abenselpar_glosry.htm'\) "Glossary Entry") or the first input field for a [selection criterion](javascript:call_link\('abenselection_criterion_glosry.htm'\) "Glossary Entry") is displayed. POS\_HIGH specifies the position at which the second input field for a selection criterion is displayed. If the statement is part of a block with a frame, a number specified for pos is interpreted with respect to the frame.