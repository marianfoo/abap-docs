...
The example shows that reading from database tables and reading from internal tables with the same content yield the same results. While the database tables are accessed on the database, the WITH statement for the internal tables can be processed on the AS ABAP since none of the restrictions listed under [ABAP SQL Engine, Restrictions](https://help.sap.com/doc/abapdocu_758_index_htm/7.58/en-US/abensql_engine_restr.htm) occur.