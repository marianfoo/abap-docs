...
This example demonstrates the access to one internal table in the statement [WITH](javascript:call_link\('abapwith.htm'\)).
...
Because of the usage of UNION, the ABAP SQL statement [WITH](javascript:call_link\('abapwith.htm'\)) can be processed on the database only and it is not possible to address more than on internal table within the statement, even if it is the same statement: The second WITH statement gives a syntax error. As a workaround, the internal table is addressed only once in an additional [common table expression](javascript:call_link\('abencommon_table_expression_glosry.htm'\) "Glossary Entry") which than is used two times in the following CTE.