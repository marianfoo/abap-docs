...
Represents an access mechanism of the [Application Server ABAP](javascript:call_link\('abenas_abap_glosry.htm'\) "Glossary Entry") providing SQL-level access to published, ABAP-managed [CDS view entities](javascript:call_link\('abencds_v2_view_glosry.htm'\) "Glossary Entry") for consumers outside the system.
