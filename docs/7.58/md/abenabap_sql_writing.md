...
-   When making writes to a DDIC database table for which a database cursor is opened in a [SELECT loop](javascript:call_link\('abapselect.htm'\)), the result set is database-dependent and undefined. This kind of parallel access should therefore be avoided.
-   The latter is also true using an [OPEN CURSOR](javascript:call_link\('abapopen_cursor.htm'\)).
