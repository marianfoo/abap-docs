...
-   Some expressions use [obsolete syntax forms](javascript:call_link\('abentokenization_obsolete.htm'\)) that can still occur outside classes, where the separators between the tokens can be omitted.
-   The absolute maximum length of an ABAP statement is 128 x 1024 characters = 131072 characters. However, the actual maximum number of characters in a statement also depends on the number of tokens and the use of literals. In general, a statement can consist of almost 130000 characters. In [chained statements](javascript:call_link\('abenchained_statement_glosry.htm'\) "Glossary Entry"), however, there are only a maximum of 28400 characters per statement.
