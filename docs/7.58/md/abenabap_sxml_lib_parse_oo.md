...
An XML reader is created using the factory method CREATE of the required class, as in [token-based parsing](javascript:call_link\('abenabap_sxml_lib_parse_iterative.htm'\)), and the source xml of the XML data is passed, for example:
...
These methods are sufficient for all simple parsing tasks. For accessing XML attributes, a node object of an element opening contains the appropriate methods. To reset the parser to a preceding node or to parse the current node with its subnodes, the methods [PUSH\_BACK](javascript:call_link\('abenabap_sxml_lib_parse_iterative.htm'\)) and [SKIP\_NODE](javascript:call_link\('abenabap_sxml_lib_parse_iterative.htm'\)) can also be used here. Further token-based parsing methods should not usually be used at the same time.
...
-   If an XML element has multiple identically named attributes, only one of these attributes is used in object-oriented parsing (unlike in [token-based parsing](javascript:call_link\('abenabap_sxml_lib_parse_iterative.htm'\))). This attribute has the value of the last identically named attribute (see example class CL\_DEMO\_XML\_ATTRIBUTES).
