...
*\[*[internal](javascript:call_link\('abenbdl_internal.htm'\))*\]**\[* [static](javascript:call_link\('abenbdl_static.htm'\))*\]**\[*factory*\]*
...
Defines a [RAP save action](javascript:call_link\('abenrap_save_action_glosry.htm'\) "Glossary Entry"). RAP save actions are [RAP actions](javascript:call_link\('abenrap_action_glosry.htm'\) "Glossary Entry") that can only be called during a specified [RAP saver method](javascript:call_link\('abenabp_saver_method_glosry.htm'\) "Glossary Entry") in the [RAP save sequence](javascript:call_link\('abenrap_save_seq_glosry.htm'\) "Glossary Entry"). Any call during the [RAP interaction phase](javascript:call_link\('abenrap_int_phase_glosry.htm'\) "Glossary Entry") leads to a [short dump](javascript:call_link\('abenshort_dump_glosry.htm'\) "Glossary Entry").

Save actions are characterized by the syntax addition save. The name of the [RAP saver method](javascript:call_link\('abenabp_saver_method_glosry.htm'\) "Glossary Entry") during which the action can be executed is specified in brackets. finalize, adjustnumbers, or both can be specified in brackets to indicate the saver method during which the action can be called.

[Non-factory actions](javascript:call_link\('abenbdl_action_nonfactory.htm'\)) and [factory actions](javascript:call_link\('abenbdl_action_factory.htm'\)) (instance-bound and static) can be defined as save action. A default factory action, however, cannot be a save action. The respective syntax rules of the kind of action apply (non-factory, static factory, instance-bound factory).

The following [RAP BDL operation additions](javascript:call_link\('abenbdl_operations_additions.htm'\)) are allowed for save actions:
...
-   From an external [RAP BO consumer](javascript:call_link\('abenrap_bo_consumer_glosry.htm'\) "Glossary Entry").
-   In the [ABAP behavior pool](javascript:call_link\('abenbehavior_pool_glosry.htm'\) "Glossary Entry"), from a [finalize](javascript:call_link\('abensaver_finalize.htm'\)) or [adjust\_numbers](javascript:call_link\('abensaver_adjust_numbers.htm'\)) RAP saver method, according to the specification in brackets.
-   A save(finalize) action can be called from the implementation of a [RAP determination](javascript:call_link\('abenrap_determination_glosry.htm'\) "Glossary Entry") on save.
    ...
    Caution: If a determination on save is executed as part of a [RAP BO determine action](javascript:call_link\('abenrap_bo_det_action_glosry.htm'\) "Glossary Entry"), the execution takes place during the interaction phase. Therefore, if a RAP save action is called in a determination on save and the determination is executed as part of a determine action, a short dump occurs.
...
-   In the implementation of a [RAP determination](javascript:call_link\('abenrap_determination_glosry.htm'\) "Glossary Entry") on modify. If this is done, a short dump occurs.
-   If called from the implementation of any other [RAP handler method](javascript:call_link\('abenabp_handler_method_glosry.htm'\) "Glossary Entry"), a short dump occurs.
...
-   [Base BDEF extensions](javascript:call_link\('abenrap_base_bdef_ext_glosry.htm'\) "Glossary Entry").
...
-   RAP save actions are required for the RAP representation of certain existing functionality which does not maintain a [transactional buffer](javascript:call_link\('abentransactional_buffer_glosry.htm'\) "Glossary Entry") but (from a RAP perspective) only has a save phase.
