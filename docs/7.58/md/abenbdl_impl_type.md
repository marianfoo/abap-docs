...
-   projection
    ...
    See also: [RAP BDL - projection behavior definitions](javascript:call_link\('abenbdl_projection_bo.htm'\)).
...
-   abstract
    ...
    See also: [RAP abstract behavior definition](javascript:call_link\('abenbdl_abstract.htm'\))
...
-   interface
    ...
    See also: [RAP Interface Behavior Definition](javascript:call_link\('abenbdl_rap_bo_interface.htm'\)).
...
The following example shows a managed BDEF based on the CDS root view entity DEMO\_SALES\_CDS\_BUPA\_2. The implementation type managed is specified in the [RAP behavior definition header](javascript:call_link\('abencds_bdef_header_glosry.htm'\) "Glossary Entry").
