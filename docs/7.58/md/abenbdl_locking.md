...
Addition That Can Be Used for an [Action](javascript:call_link\('abenbdl_action.htm'\))
...
The class CL\_DEMO\_SALES\_RAP\_LOCK accesses the business object using [EML](javascript:call_link\('abeneml_glosry.htm'\) "Glossary Entry") and creates two BO instances.
...
The class CL\_DEMO\_RAP\_UNMANAGED\_LOCK accesses the business object using [EML](javascript:call_link\('abeneml_glosry.htm'\) "Glossary Entry") and carries out the following steps:
...
The class CL\_DEMO\_RAP\_UNMANAGED\_DRFT\_LCK accesses the business object using [EML](javascript:call_link\('abeneml_glosry.htm'\) "Glossary Entry") and performs the following steps:
...
Can be used as syntax addition for an [action](javascript:call_link\('abenbdl_action.htm'\)) to prevent the locking mechanism for the entity instance for which an action is executed. See topic [RAP - action](javascript:call_link\('abenbdl_action.htm'\)).