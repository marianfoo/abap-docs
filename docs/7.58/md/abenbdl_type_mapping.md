...
The [assignment and conversion rules](javascript:call_link\('abenconversion_rules.htm'\)) for elementary data objects apply to the mapping of components.
...
For details on extensibility enabling, see topic [RAP - Extensibility Enabling for Base BOs](javascript:call_link\('abenbdl_extensibility_enabling_m_u.htm'\)).