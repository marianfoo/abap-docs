...
-   The operation fails and the complete content of the [transactional buffer](javascript:call_link\('abentransactional_buffer_glosry.htm'\) "Glossary Entry") of the current [RAP transaction](javascript:call_link\('abenrap_luw_glosry.htm'\) "Glossary Entry") is rejected. A commit to the database happens only if all data changes are consistent. One inconsistency leads to a rejection of all the content in the transactional buffer.
...
The example above is explained in detail in the executable example [RAP BDL - validation](javascript:call_link\('abenbdl_validation_abexa.htm'\)).

Continue
![Example](exa.gif "Example") [RAP - Validation](javascript:call_link\('abenbdl_validation_abexa.htm'\))