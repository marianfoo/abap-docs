...
These [built-in functions](javascript:call_link\('abenbuilt_in_functions.htm'\)) return the character string from [text](javascript:call_link\('abenstring_functions_val.htm'\)) after it has been converted in accordance with the following case rules:
