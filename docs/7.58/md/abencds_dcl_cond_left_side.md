...
-   The value of a [session variable](javascript:call_link\('abensession_variable_glosry.htm'\) "Glossary Entry") replaced by $session.vname
...
-   Left side host expressions cannot be used on the left side of [PFCG mappings](javascript:call_link\('abencds_pfcg_mapping_glosry.htm'\) "Glossary Entry") or user-defined aspects.
