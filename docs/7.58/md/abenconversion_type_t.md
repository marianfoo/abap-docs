...
description: |
  The conversion rules are designed in such a way that, when data objects of time type t(https://help.sap.com/doc/abapdocu_758_index_htm/7.58/en-US/abenbuiltin_types_date_time.htm) are assigned to character-like data objects, they behave like character-like objects and numerically in assignments to
version: "7.58"
category: "types"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_758_index_htm/7.58/en-US/abenconversion_type_t.htm"
...
If the source field contains only digits, the content is interpreted as a time specification in the format hhmmss from which the value hh\*3600+mm\*60+ss is calculated and then converted to the internal representation of the corresponding integer. If the source field does not only contain digits, the target field is given the value 0. If the [value range](https://help.sap.com/doc/abapdocu_758_index_htm/7.58/en-US/abenvalue_range_glosry.htm "Glossary Entry") of the internal data types b and s is not sufficient, the catchable exception CX\_SY\_CONVERSION\_OVERFLOW is raised.
...
If the source field contains only digits, the content is interpreted as a time specification in the format hhmmss from which the value hh\*3600+mm\*60+ss is calculated and then converted to the internal representation of a decimal floating point number with [scaling](https://help.sap.com/doc/abapdocu_758_index_htm/7.58/en-US/abenscale_glosry.htm "Glossary Entry") of 0. If the source field does not contain only digits, the target field is given the value 0.
