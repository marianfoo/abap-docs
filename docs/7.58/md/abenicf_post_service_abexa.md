...
This example demonstrates calls of an HTTP service in [ICF](https://help.sap.com/doc/abapdocu_758_index_htm/7.58/en-US/abenicf_glosry.htm "Glossary Entry") and a Web browser. The input data is passed using the POST method.
...
This example has the same structure as the executable example [Calling an HTTP Service](https://help.sap.com/doc/abapdocu_758_index_htm/7.58/en-US/abenicf_service_abexa.htm). In this case, the method IF\_HTTP\_EXTENSION~HANDLE\_REQUEST of the class CL\_HTTP\_EXT\_SERVICE\_POST\_DEMO is used to demonstrate how user input sent by a HTML page form generated by the handler class is sent using the POST method and read using the method GET\_CDATA.
