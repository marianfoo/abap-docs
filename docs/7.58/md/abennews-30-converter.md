...
When Utilities → Typing is chosen in ABAP Editor, a conversion program is called which types the formal parameters of all [FORM](https://help.sap.com/doc/abapdocu_758_index_htm/7.58/en-US/abapform.htm) routines of an ABAP program and its includes.
...
If there are multiple [compilation units](https://help.sap.com/doc/abapdocu_758_index_htm/7.58/en-US/abencompilation_unit_glosry.htm "Glossary Entry") for an include, this is indicated at the end of the list. Conversion proposals are still made, but they refer only to the current compilation unit. The names of the FORM routines defined in this include appear in the list of conversion proposals underlined in red.
