...
As of ABAP release 7.0, EhP2, all documents for printing are formatted in HTML. The individual executable example programs from [ABAP examples](javascript:call_link\('abenabap_examples.htm'\)) and the [ABAP glossary](javascript:call_link\('abenabap_glossary.htm'\)) can now also be printed. In addition, tabular displays are now aligned correctly on printouts.
