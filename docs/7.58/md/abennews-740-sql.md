...
In ABAP release 7.40 and higher, special access types for entities in the [SAP HANA database](javascript:call_link\('abenhana_database_glosry.htm'\) "Glossary Entry") are possible (if this database is used as the [standard database](javascript:call_link\('abenstandard_db_glosry.htm'\) "Glossary Entry") for an AS ABAP).
