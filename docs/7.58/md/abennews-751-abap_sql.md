...
In the additions named under [additional\_options](javascript:call_link\('abapselect_additions.htm'\)) in the statement [SELECT](javascript:call_link\('abapselect.htm'\)), an addition [OFFSET](javascript:call_link\('abapselect_up_to_offset.htm'\)) can now be specified to specify the first row of the results set.
