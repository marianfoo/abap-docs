...
A new type of [access rule](javascript:call_link\('abencds_dcl_role_rules.htm'\)) is available for CDS projection views of type [CDS transactional query](javascript:call_link\('abencds_transactional_pv_glosry.htm'\) "Glossary Entry"): the [projection\_rule](javascript:call_link\('abencds_dcl_role_proj_rule.htm'\)) defined with the statement [GRANT SELECT ON ... AS PROJECTION ON ... FALLBACK ASSOCIATION ...](javascript:call_link\('abencds_dcl_role_proj_rule.htm'\)).
