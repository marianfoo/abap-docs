...
This example has the same result as the executable example for a [difference with aggregate expression](https://help.sap.com/doc/abapdocu_758_index_htm/7.58/en-US/abenselect_except_min_abexa.htm). Here, however, the SELECT statement is split into two ABAP SQL statements for demonstration purposes:
...
-   [EXCEPT](https://help.sap.com/doc/abapdocu_758_index_htm/7.58/en-US/abapunion.htm) is used to create the difference of the result set of a SELECT statement on the table SFLIGHT and a SELECT statement on the GTT.
