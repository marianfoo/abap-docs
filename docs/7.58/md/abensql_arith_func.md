...
The following table shows the numeric functions that can be specified as SQL expressions and the requirements on the arguments. The meaning of the functions can be found under [SQL Functions for Numeric Values](javascript:call_link\('abensql_functions_numeric.htm'\)). The value x in the ABAP SQL Engine column indicates that the function can be executed by the [ABAP SQL engine](javascript:call_link\('abenabap_sql_engine_glosry.htm'\) "Glossary Entry") and that the use of [this function](javascript:call_link\('abensql_engine_expr.htm'\)) does not bypass [table buffering](javascript:call_link\('abentable_buffering_glosry.htm'\) "Glossary Entry") does not cause the transport of an internal table accessed with [FROM @itab](javascript:call_link\('abapselect_itab.htm'\)) to the database.
...
pos: INT1, INT2, INT4, and [null expression](javascript:call_link\('abensql_null.htm'\))
