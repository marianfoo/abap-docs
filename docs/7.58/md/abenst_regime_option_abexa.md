...
This example passes various types of ABAP data to the transformation DEMO\_ST\_REGIME\_OPTION and displays the result for XML and [JSON](javascript:call_link\('abenabap_json.htm'\)). The transformation applies the option [regime](javascript:call_link\('abenst_option_format.htm'\)) to the ABAP data:
