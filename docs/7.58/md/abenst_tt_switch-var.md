...
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_758_index_htm/7.58/en-US/abenst_tt_switch-var.htm"
...
The statement tt:switch-var allows case distinctions in which, unlike tt:switch, only cases for data content can be specified, not for data flow. In tt:switch, a list of cases can be shown where the syntax of each case is formulated by a condition for variables, namely a [tt:cond-var](https://help.sap.com/doc/abapdocu_758_index_htm/7.58/en-US/abenst_tt_cond-var.htm) subelement. Other direct subelements are not possible in tt:switch-var.

No more than one case can be specified that does not contain a [check](https://help.sap.com/doc/abapdocu_758_index_htm/7.58/en-US/abenst_check.htm) condition.
...
Unlike the general case distinction with [tt:switch](https://help.sap.com/doc/abapdocu_758_index_htm/7.58/en-US/abenst_tt_switch.htm), the content of the cases is irrelevant.

Example

The following transformation DEMO\_ST\_SWITCH\_VAR expands the example from [Conditions for Variables](https://help.sap.com/doc/abapdocu_758_index_htm/7.58/en-US/abenst_tt_cond-var.htm).
