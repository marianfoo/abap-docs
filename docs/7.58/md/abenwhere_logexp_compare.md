...
-   In the case of columns that contain nothing but a blank character, it depends on the platform how this blank is saved:
    ...
    -   In all other databases and in the [table buffer](javascript:call_link\('abentable_buffering_glosry.htm'\) "Glossary Entry"), a single blank is saved in accordance with the code page.
