...
-   The following applies to operand1:
    ...
    -   The data type can be one of the character-like [dictionary types](javascript:call_link\('abenddic_builtin_types.htm'\)) CHAR, NUMC, CLNT, LANG, DATS, TIMS, ACCP, CUKY, UNIT, or SSTRING.
...
-   The following applies to operand2:
    ...
    -   The content can have a maximum of twice as many characters as the length of operand1 but no more than 1333 characters. [Character literals](javascript:call_link\('abencharacter_literal_glosry.htm'\) "Glossary Entry") and [constants](javascript:call_link\('abenconstant_glosry.htm'\) "Glossary Entry") respect any trailing blanks and a syntax error occurs if this rule is broken. All other data objects ignore trailing blanks and a runtime error occurs if this rule is broken.
...
The addition ESCAPE can be used to define a single-character [escape character](javascript:call_link\('abenescape_character_glosry.htm'\) "Glossary Entry"). esc expects a [flat](javascript:call_link\('abenflat_glosry.htm'\) "Glossary Entry") [character-like](javascript:call_link\('abencharlike_data_object_glosry.htm'\) "Glossary Entry") data object with the length 1 containing the escape character. A [literal](javascript:call_link\('abenabap_sql_literals.htm'\)) or a [host variable](javascript:call_link\('abenabap_sql_host_variables.htm'\)) can be specified. In the pattern in operand2, the escape character in esc may only be placed in front of a wildcard character or in front of the escape character itself. In this case, they lose their special meaning.
