...
Structure for the properties of the screen element of the current column. The components can be set for the values described there either directly or using [MODIFY SCREEN](javascript:call_link\('abapmodify_screen.htm'\)). MODIFY SCREEN overwrites a direct assignment.
...
Flag (X or blank) indicating whether the column is selected. Is set to current value at time of [PAI](javascript:call_link\('abenpai_glosry.htm'\) "Glossary Entry").
