...
-   In the [declaration part](javascript:call_link\('abendeclaration_part_glosry.htm'\) "Glossary Entry") of [global classes](javascript:call_link\('abenglobal_class_glosry.htm'\) "Glossary Entry") the character representation of default values is limited to 132 characters. All positions exceeding 132 characters will be truncated when working in the form-based class builder in SE24. When you work in the source code-based view in SE24 and in [ADT](javascript:call_link\('abenabap_dev_envir_glosry.htm'\) "Glossary Entry"), exceeding characters will not be truncated. However, a syntax warning appears.
...
A class attribute defined using READ-ONLY can be used outside of the class, its friends, and subclasses only in [read positions](javascript:call_link\('abenread_position_glosry.htm'\) "Glossary Entry") in ABAP statements.
