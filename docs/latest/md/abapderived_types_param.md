...
The example [Using %tky](javascript:call_link\('abenderived_types_tky_abexa.htm'\)) focuses on the use of [%tky](javascript:call_link\('abapderived_types_tky.htm'\)) with a managed RAP BO in a draft and non-draft scenario. In the code snippet, the [draft action](javascript:call_link\('abenbdl_draft_action.htm'\)) edit includes the specification of %param-preserve\_changes.