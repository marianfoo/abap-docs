...
Used for the mapping of a [RAP input derived type](javascript:call_link\('abenrap_input_der_type_glosry.htm'\) "Glossary Entry") (in\_d\_type) to a non-BDEF derived type (ntype). As a prerequisite, the [mapping](javascript:call_link\('abenbdl_type_mapping.htm'\)) for the non-BDEF derived type must be specified in the BDEF.
...
Used for the mapping of a non-BDEF derived type (ntype) to a [RAP output derived type](javascript:call_link\('abenrap_output_der_type_glosry.htm'\) "Glossary Entry") (out\_d\_type). As a prerequisite, the [mapping](javascript:call_link\('abenbdl_type_mapping.htm'\)) for the non-BDEF derived type must be specified in the BDEF.
