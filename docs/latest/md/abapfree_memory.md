...
The use of FREE MEMORY without the addition ID is [obsolete](javascript:call_link\('abapfree_mem_id_obsolete.htm'\)). While the statement EXPORT without addition ID affects only one data cluster, for FREE MEMORY, all clusters are affected. It is safest to use the statement DELETE FROM, because here the addition ID is mandatory.
