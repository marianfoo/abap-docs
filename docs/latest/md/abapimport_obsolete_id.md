...
Outside of classes, the addition ID of the statement [IMPORT](javascript:call_link\('abapimport_data_cluster.htm'\)) [FROM DATABASE](javascript:call_link\('abapimport_medium.htm'\)) can be replaced by these additions when database tables are imported. When id1 and id2 are specified, the same rules apply as to id.
