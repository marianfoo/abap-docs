...
description: |
  Short Reference(https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abapreplace_shortref.htm) Syntax ... VERBATIM RESPECTINGIGNORING CASE REPLACEMENT COUNT rcnt  REPLACEMENT LINE rlin REPLACEMENT OFFSET roff REPLACEMENT LENGTH rlen
version: "latest"
...
The addition REPLACEMENT LINE returns the last line number in which the search pattern was found using the statement [REPLACE IN TABLE](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abapreplace_itab.htm) in rlin. The following can be specified for rlin:
...
The remaining additions used for making replacements in individual table lines have the same [meaning](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abapreplace_options.htm) as pattern-based replacements made using [REPLACE](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abapreplace_in_pattern.htm) in elementary character strings or byte strings.
...
This example performs the same replacement as the [replacement range](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abapreplace_table_range.htm) example. Here, the positions of the replaced occurrences are returned.
