...
This statement determines whether the blank lines created using [WRITE](javascript:call_link\('abapwrite-.htm'\)) are displayed. If the addition ON is specified, all subsequent lines created using WRITE statements are written in the list. If the addition OFF is specified (default), all subsequent lines that contain only blanks after a line break are not written to the list.
