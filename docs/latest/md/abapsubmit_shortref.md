...
-   [USING SELECTION-SET variant](javascript:call_link\('abapsubmit_selscreen_parameters.htm'\))
    Supplies the selection screen with values for a [selection screen variant](javascript:call_link\('abenvariant_glosry.htm'\) "Glossary Entry") variant.
-   [USING SELECTION-SETS OF PROGRAM prog](javascript:call_link\('abapsubmit_selscreen_parameters.htm'\))
    Specifies a program prog whose [selection screen variants](javascript:call_link\('abenvariant_glosry.htm'\) "Glossary Entry") are used.
...
-   [WITH sel1 ... WITH sel2 ...](javascript:call_link\('abapsubmit_selscreen_parameters.htm'\))
    ...
    *{*EQ*|*NE*|*CP*|*NP*|*GE*|*LT*|*LE*|*GT*}* dobj *\[*SIGN sign*\]* - Passes a single value dobj with comparison operators and a specification for the SIGN column in the [selection table](javascript:call_link\('abenselection_table_glosry.htm'\) "Glossary Entry").
    *\[*NOT*\]* BETWEEN dobj1 AND dobj2 *\[*SIGN sign*\]* - Passes an interval from dobj1 to dobj2 with an optional operator NOT and a specification for the SIGN column in the selection table.
    IN range\_tab - Passes a [ranges table](javascript:call_link\('abenranges_table_glosry.htm'\) "Glossary Entry") range\_tab.
-   [WITH FREE SELECTIONS texpr](javascript:call_link\('abapsubmit_selscreen_parameters.htm'\))
    Supplies the dynamic selection for the selection screen on a logical database with values from an internal table texpr of type rsds\_texpr from [type pool](javascript:call_link\('abentype_pool_glosry.htm'\) "Glossary Entry") RSDS.
...
-   [EXPORTING LIST TO MEMORY](javascript:call_link\('abapsubmit_list_options.htm'\))
    Saves the basic list for the program accessed as an internal table of the line type ABAPLIST in the [ABAP Memory](javascript:call_link\('abenabap_memory_glosry.htm'\) "Glossary Entry"). Can only be used together with AND RETURN.
