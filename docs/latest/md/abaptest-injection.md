...
description: |
  Short Reference(https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abaptest-injection_shortref.htm) Syntax TEST-INJECTION seam. statement_block END-TEST-INJECTION. Effect The statement TEST-INJECTION introduces an injection(https://help.sap.com/doc/abapdocu_latest_index_htm
version: "latest"
category: "general"
type: "abap-reference"
...
[Short Reference](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abaptest-injection_shortref.htm)
...
The statement TEST-INJECTION introduces an [injection](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abeninjection_glosry.htm "Glossary Entry") for a [test seam](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abentest_seam_glosry.htm "Glossary Entry") seam, which is closed using END-TEST-INJECTION. The test seam seam must be defined in the production code of the current program using the statement [TEST-SEAM](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abaptest-seam.htm). When a unit test is performed, the statement block statement\_block of the injection replaces the statement block of the test seam seam in production code.

An injection can only be defined in the methods of test classes created in a [test include](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abentest_include_glosry.htm "Glossary Entry"). Injections have the following properties:
...
-   An injection cannot extend beyond the limits of a [statement block](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenstatement_block_glosry.htm "Glossary Entry") but can contain closed [control structures](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abencontrol_structure_glosry.htm "Glossary Entry").
-   An injection can include local data declarations using the statement [DATA](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abapdata.htm). These variables are visible below their declaration in the current injection and all following injections of the current test class. The data objects declared in an injection are not visible outside of injections, that is, neither in other parts of the test class nor in production code. Injections that access data objects declared in a different injection can only be assigned to test seams defined in the production source code below the test seam, that is, the test seam assigned to the declaring injection.
...
-   The injections for a test seam must be defined in the same [compilation unit](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abencompilation_unit_glosry.htm "Glossary Entry").
...
The test method tests the method CHANGE\_PRICE (see the example for [TEST-SEAM](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abaptest-seam.htm)). Before the test method is executed, the injection replaces the injection in the setup method of the test seam selection. When the test method is executed,
...
Continue
[END-TEST-INJECTION](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abapend-test-injection.htm)