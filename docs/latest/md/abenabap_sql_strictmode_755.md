...
The strict mode of the syntax check in ABAP release 7.55 applies to all ABAP SQL statements that use one of the following properties introduced in [ABAP release 7.55](javascript:call_link\('abennews-755-abap_sql.htm'\)):
...
The strict mode in ABAP release 7.55 covers all rules of the [strict mode in ABAP release 7.54](javascript:call_link\('abenabap_sql_strictmode_754.htm'\)).