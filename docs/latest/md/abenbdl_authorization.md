...
In a [projection business object](javascript:call_link\('abenrap_projection_bo_glosry.htm'\) "Glossary Entry"), the authorization from the base business object is automatically inherited. If [new actions or functions](javascript:call_link\('abenbdl_nonstandard_projection.htm'\)) are defined in the projection layer, a new authorization control for these actions and functions can be defined. For further details, see topic [RAP - authorization, Projection BDEF](javascript:call_link\('abenbdl_authorization_projection.htm'\)).
...
-   To limit read access to a business object, [Data Control Language (DCL)](javascript:call_link\('abendcl_glosry.htm'\) "Glossary Entry") can be used. These CDS access controls defined in [Data Control Language (DCL)](javascript:call_link\('abendcl_glosry.htm'\) "Glossary Entry") are passed on from the base layer to the RAP BO projection and the RAP BO interface. An executable example that demonstrates how read access control is implemented and evaluated can be found in topic [RAP BDL - RAP BO with DCL Access Control](javascript:call_link\('abenbdl_draft_query_abexa.htm'\)).
...
The class CL\_DEMO\_RAP\_MANAGED\_AUTHRZTN accesses the business object using [EML](javascript:call_link\('abeneml_glosry.htm'\) "Glossary Entry") and performs the following actions:
...
The class CL\_DEMO\_RAP\_UNMANAGED\_AUTH accesses the business object using [EML](javascript:call_link\('abeneml_glosry.htm'\) "Glossary Entry") and performs the following actions:
...
The example above is explained in detail in the executable example [RAP BDL - global authorization](javascript:call_link\('abenbdl_authorization_abexa.htm'\)).
...
For details, see topic [RAP - syntax\_short\_form](javascript:call_link\('abenbdl_short_syntax.htm'\)).
...
Can be used as [RAP BO operation addition](javascript:call_link\('abenbdl_operations_additions.htm'\)) to exclude the operation in question from authorization checks. See [RAP BDL - authorization:none](javascript:call_link\('abenbdl_actions_auth.htm'\)).
...
Can be used as [RAP BO operation addition](javascript:call_link\('abenbdl_operations_additions.htm'\)) to delegate the authorization check. An operation specified as authorization:update has the same authorization control that is defined for the update operation. See [RAP BDL - authorization:update](javascript:call_link\('abenbdl_actions_auth_update.htm'\)).
...
Can be used as [RAP BO operation addition](javascript:call_link\('abenbdl_operations_additions.htm'\)) to replace the authorization control that is specified in the authorization master entity and apply global authorization checks instead. See [RAP BDL - authorization:global](javascript:call_link\('abenbdl_actions_auth_global.htm'\)).
...
Can be used as [RAP BO operation addition](javascript:call_link\('abenbdl_operations_additions.htm'\)) to replace the authorization control that is specified in the authorization master entity and apply instance authorization checks instead. See [RAP BDL - authorization:instance](javascript:call_link\('abenbdl_actions_auth_instance.htm'\)).

Continue
![Example](exa.gif "Example") [RAP - Global Authorization](javascript:call_link\('abenbdl_authorization_abexa.htm'\))