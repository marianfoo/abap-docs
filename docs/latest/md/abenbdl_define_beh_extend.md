...
... [define behavior for ExtNode *\[*using*\]*](javascript:call_link\('abenbdl_define_beh.htm'\)) ...

Prerequisite

As a prerequisite, the extended [RAP behavior definition](javascript:call_link\('abencds_behavior_definition_glosry.htm'\) "Glossary Entry") must explicitly allow BDEF extensions as described in topic [RAP BDL - Extensibility Enabling for Base BOs](javascript:call_link\('abenbdl_extensibility_enabling_m_u.htm'\)).
...
-   A [CDS transactional interface](javascript:call_link\('abencds_trans_interface_glosry.htm'\) "Glossary Entry") can be specified with the syntax addition [using](javascript:call_link\('abenbdl_using_interface.htm'\)) (instead of [using interface](javascript:call_link\('abenbdl_using_interface.htm'\)), which is used for RAP BO extensions).
...
If an ABAP behavior pool is specified, the rules for extension ABPs apply. See topic [RAP BDL - implementation in class unique, Extension](javascript:call_link\('abenbdl_in_class_unique_ext.htm'\)).
...
-   Currently, [RAP business events](javascript:call_link\('abenrap_entity_event_glosry.htm'\) "Glossary Entry") can only be defined in [RAP BO root entities](javascript:call_link\('abenrap_bo_root_entity_glosry.htm'\) "Glossary Entry"). Since extension nodes are usually below the root node, they cannot define any new RAP business events.
...
Continue
[RAP - ancestor association](javascript:call_link\('abenbdl_ancestor_ext.htm'\))