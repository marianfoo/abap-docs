...
Defines managed [internal numbering](javascript:call_link\('abenrap_int_numbering_glosry.htm'\) "Glossary Entry") for a [RAP BO field](javascript:call_link\('abenrap_bo_field_glosry.htm'\) "Glossary Entry"). The field in question is automatically assigned values upon creation of a new entity instance. The RAP BO consumer does not have to provide any input for those fields.
...
For user-defined numbering, the keyword early numbering is available, see topic [RAP BDL - early numbering](javascript:call_link\('abenbdl_early_numb.htm'\)).
...
The ABAP class CL\_DEMO\_RAP\_MANAGED\_FIELDS uses [EML](javascript:call_link\('abeneml_glosry.htm'\) "Glossary Entry") to access to [RAP business object](javascript:call_link\('abenrap_bo_glosry.htm'\) "Glossary Entry") and performs the following steps:
