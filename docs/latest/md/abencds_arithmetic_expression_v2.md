...
-   [SQL-based scalar functions](javascript:call_link\('abencds_sql_scalar_function.htm'\)) that return a numeric type
...
-   If an operand of an arithmetic expression has the [null value](javascript:call_link\('abennull_value_glosry.htm'\) "Glossary Entry"), the result of the entire arithmetic expression is the null value.
