...
The following CDS view works in exactly the same way as the [DDIC projection view](javascript:call_link\('abenddic_projection_views.htm'\)) DEMO\_SPFLI in ABAP Dictionary. The class CL\_DEMO\_CDS\_PROJECTION uses SELECT to access the view. Unlike when the DDIC view DEMO\_SPFLI is accessed, no client column is returned when the CDS entity DEMO\_CDS\_SPFLI is accessed. The CDS-managed DDIC view (obsolete) DEMO\_CDS\_PRJCTN returns the client column too.
