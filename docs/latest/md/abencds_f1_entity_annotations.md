...
Allows node extensibility. In other words, explicitly allows new child nodes to be added by means of [to-child associations](javascript:call_link\('abento_child_association_glosry.htm'\) "Glossary Entry").
...
Specifies whether a placeholder ? or a literal value is passed to the database in an [ABAP SQL condition](javascript:call_link\('abenasql_cond.htm'\)) when the client field is compared with a host variable.
...
The annotation Environment.sql.passValueForClient is available in [CDS hierarchies](javascript:call_link\('abencds_hierarchy_glosry.htm'\) "Glossary Entry"), [CDS view entities](javascript:call_link\('abencds_v2_view_glosry.htm'\) "Glossary Entry"), and in [CDS projection views](javascript:call_link\('abencds_projection_view_glosry.htm'\) "Glossary Entry"). It is not available in any other CDS entity.
...
-   In CDS custom entities, the annotation ObjectModel.query.implementedBy is mandatory when working in [ABAP Cloud](javascript:call_link\('abenabap_cloud_glosry.htm'\) "Glossary Entry"). In all other environments and for all other CDS entities, it is optional.
