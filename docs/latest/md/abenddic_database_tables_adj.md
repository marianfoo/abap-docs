...
-   When a [client column](javascript:call_link\('abenclient_column_glosry.htm'\) "Glossary Entry") is inserted. During the adjustment, the data of the table is copied to all clients listed in the client table T000.
...
-   In adjustments, initial fields are filled with their type-dependent initial value, regardless of the [flag for initial values](javascript:call_link\('abenddic_database_tables_init.htm'\)).
