...
An additional type property that is not one of the [technical type properties](javascript:call_link\('abentechnical_type_prpt_glosry.htm'\) "Glossary Entry") and thus is ignored by the [compatibility checks](javascript:call_link\('abencompatible_glosry.htm'\) "Glossary Entry") is the [initial memory requirement](javascript:call_link\('abeninitial_mem_req_glosry.htm'\) "Glossary Entry"). When defining a table type, the addition [INITIAL SIZE](abaptypes_itab.htm#!ABAP_ONE_ADD@1@) can be used to affect the initial memory requirement.
