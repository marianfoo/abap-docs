...
-   When specifying the primary table key using primary\_key, it is important to note that it can also be [empty](javascript:call_link\('abenitab_empty_key.htm'\)) for standard tables. This can produce unexpected behavior in statements where the key is used to specify the lines to be processed.
