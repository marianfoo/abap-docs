...
-   The [ABAP system fields](javascript:call_link\('abensystem_fields.htm'\)) are located outside of the contexts listed above. They are hidden by global program data objects with the same names.
-   The same is true for the obsolete structure [screen](javascript:call_link\('abenscreen_structure_obsolete.htm'\)).
