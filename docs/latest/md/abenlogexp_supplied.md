...
-   [Remote-enabled function modules](javascript:call_link\('abenremote_enabled_fm_glosry.htm'\) "Glossary Entry")
    -   In a [remote function call](javascript:call_link\('abapcall_function_destination-.htm'\)) between two AS ABAP, both must have at least ABAP release 4.6 so that IS SUPPLIED can be used in the called function module.
