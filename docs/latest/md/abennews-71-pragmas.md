...
From ABAP release 7.0, EhP2, [pragmas](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenpragma.htm) can be used to hide warnings from the ABAP Compiler syntax check and other check tools.

Modification 2   

Switching from Pseudo Comments

The [pseudo comments](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenpseudo_comment_glosry.htm "Glossary Entry") "#EC ..., which until now could be used to hide warnings from [enhanced program checks](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenextended_program_check_glosry.htm "Glossary Entry"), were to be replaced by the ##... pragmas. The statement [SET EXTENDED CHECK](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abapset_extended_check.htm) must no longer be used in programs in which a warning from the extended program check is hidden by a pragma. If used, the statement produces a warning that cannot be hidden.