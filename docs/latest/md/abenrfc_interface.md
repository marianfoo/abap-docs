...
-   Converting actual parameters to the format required in the remote system and vice-versa. This includes any platform-dependent conversions, for example, for different [code pages](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abencodepage_glosry.htm "Glossary Entry") or byte orders. The conversion supports all ABAP data types.
-   Handling of errors that occur during communication and forwarding them to the caller if [requested](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenrfc_exception.htm).
