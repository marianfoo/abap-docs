...
The second trap is common for all [comparisons of character-like data](abencomparison_type.htm#@@ITOC@@ABENCOMPARISON_TYPE_2). Comparisons are based on the internal binary representation that depends on the code page. The [locale](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenlocale_glosry.htm "Glossary Entry") or the current [text environment](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abentext_environment.htm) are not taken into account. If this is not taken into account, when evaluating input from selection screens this can lead to unexpected results. The example shows a selection criterion selcrit that can deal with lower case letters but is restricted to the normal comparison rules. If an interval A,Bz is entered:

-   The string Bar is found in the interval using the operator [IN](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenlogexp_select_option.htm).
-   The string Bär is not found using the operator [IN](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenlogexp_select_option.htm) because the internal representation of German umlaut ä is not part of the interval.