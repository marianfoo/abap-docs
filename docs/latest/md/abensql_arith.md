...
-   Arithmetic expressions can be calculated by the [ABAP SQL engine](javascript:call_link\('abenabap_sql_engine.htm'\)) for accessing the [table buffer](javascript:call_link\('abentable_buffer_glosry.htm'\) "Glossary Entry") and internal tables with [FROM @itab](javascript:call_link\('abapselect_itab.htm'\))..
