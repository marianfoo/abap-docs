...
-   A string expression cannot be evaluated by the [ABAP SQL engine](javascript:call_link\('abenabap_sql_engine.htm'\)) and always causes [table buffer](javascript:call_link\('abentable_buffer_glosry.htm'\) "Glossary Entry") to be bypassed and an internal table accessed by [FROM @itab](javascript:call_link\('abapselect_itab.htm'\)) must be transported to the database. This is only possible for one internal table per ABAP SQL statement.
