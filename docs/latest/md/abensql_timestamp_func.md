...
The use of the functions UTCL\_CURRENT( ), UTCL\_ADD\_SECONDS, and UTCL\_SECONDS\_BETWEEN enforces the [strict mode from ABAP release 7.55](javascript:call_link\('abenabap_sql_strictmode_755.htm'\)).
...
The use of the functions TSTMP\_IS\_VALID, TSTMP\_CURRENT\_UTCTIMESTAMP( ), TSTMP\_SECONDS\_BETWEEN, and TSTMP\_ADD\_SECONDS enforces the [strict mode from ABAP release 7.53](javascript:call_link\('abenabap_sql_strictmode_753.htm'\)).
