...
Conditional transformations are realized as the content of an element tt:\[s-|d-\]cond. They represent parts of ST programs that are respected only if certain prerequisites are met. Conditional transformations are normally used as cases in [tt:switch](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenst_tt_switch.htm) and [tt:group](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenst_tt_group.htm).

The content of tt:*\[*s-*|*d-*\]*cond depends on

-   a precondition in the attribute [using](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenst_using.htm)
-   an assertion in the attribute [data](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenst_data.htm)
-   a condition in the attribute [check](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenst_check.htm)

If the content of tt:*\[*s-*|*d-*\]*cond is not a [pattern](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenst_pattern.htm), at least one attribute using, data, or check must be specified.
...
The condition [check](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenst_check.htm) can also be used directionally:
...
-   [precondition using](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenst_using.htm)
-   the [assertion data](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenst_data.htm)
-   the [condition check](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenst_check.htm)
...
In deserializations, before and during the check of the [precondition](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenst_using.htm), the system distinguishes whether or not the content of tt:*\[*d-*\]*cond is a [pattern](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenst_pattern.htm). The deserialization proceeds as follows:

1.  Check whether the content of tt:*\[*d-*\]*cond is a [pattern](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenst_pattern.htm).
    -   If the content of tt:*\[*d-*\]*cond is a [pattern](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenst_pattern.htm), it is compared to the current element of the XML inbound stream. If the pattern does not fit the current element, the element tt:*\[*d-*\]*cond is skipped and the current element of the XML inbound stream is not consumed. Otherwise Step 2 follows.
    -   If the content of tt:*\[*d-*\]*cond is not a [pattern](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenst_pattern.htm), Step 2 follows.
2.  Check of the [precondition using](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenst_using.htm)
    -   If the content of tt:*\[*d-*\]*cond is a [pattern](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenst_pattern.htm) and the precondition is not met, the element tt:*\[*d-*\]*cond is skipped and the current element of the inbound XML stream is consumed without being deserialized. If the precondition is met, Step 3 follows.
    -   If the content of tt:*\[*d-*\]*cond is not a [pattern](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenst_pattern.htm) and the precondition is not met, the deserialization is canceled with the exception CX\_ST\_REF\_ACCESS. If the precondition is met, Step 3 follows.
...
4.  Establishment of the [assertion data](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenst_data.htm)
...
5.  Check of the [condition check](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenst_check.htm)
...
If none of the three possible attributes using, data, or check is specified in tt:*\[*d-*\]*cond, the content of tt:*\[*d-*\]*cond must not be empty and it is evaluated in Step 1, depending on whether or not it is a [pattern](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenst_pattern.htm). The other prerequisites (Steps 3 to 4) are considered fulfilled.
...
Continue
[ST - using, Preconditions](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenst_using.htm)
[ST - data, Assertions](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenst_data.htm)
[ST - check, Conditions](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenst_check.htm)
[ST - Pattern](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenst_pattern.htm)
[ST - tt:empty, Empty Content](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenst_tt_empty.htm)