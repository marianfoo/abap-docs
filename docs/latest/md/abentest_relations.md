...
-   kind must be used to specify the type of some repository objects and must be omitted for others. The following can be used for kind:
    ...
    kind cannot and must not be specified for other repository objects such as global classes and [CDS entities](javascript:call_link\('abencds_entity_glosry.htm'\) "Glossary Entry"). They are addressed by their names alone.
...
-   Test relations are not considered, when starting the Unit tests of an ABAP program directly from the [repository browser](javascript:call_link\('abenrepository_browser_glosry.htm'\) "Glossary Entry") or an editor of the [ABAP Workbench](javascript:call_link\('abenabap_workbench_glosry.htm'\) "Glossary Entry") in SAP GUI.
